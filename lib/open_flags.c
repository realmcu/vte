...
parse_open_flags(char *string, char **badname)
{
   ...
   while ( 1 ) {
      ...
      *cc  '\0';
      ...
      for(ind0; ind < sizeof(Open_flags)/sizeof(struct open_flag_t); ind++) {
...
openflags2symbols(int openflags, char *sep, int mode)
{
    ...
    sizesizeof(Open_flags)/sizeof(struct open_flag_t);
...
char **argv;
{
 ...
 retparse_open_flags(argv[1], &err);
 if ( ret  -1 )
