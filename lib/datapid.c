...
printf("partial at beginning, cnt  %d, woff  %d\n", cnt, woff);
 ...
 for (tmp0; tmp<cnt; tmp++) {   /* skip unused bytes */
     chr++;
        }
 ...
 for (; boff<(NBPW-cnt) && boff<bsize; boff++, chr++) {
    ...
    for (cnt0; cnt<num_full_words; woff + NBPW, cnt++ ) {

 word  ((LOWER16BITS(pid) << 48) | (LOWER32BITS(woff) << 16) | LOWER16BITS(pid));

 chr  (char *)&word;
 for(tmp0; tmp<NBPW; tmp++, chr++) {
     buffer[boff++]  *chr;
 }
...
printf("partial at end\n");
 ...
 for (tmp0; tmp<cnt && boff<bsize; tmp++, chr++) {
     buffer[boff++]  *chr;
 }
    }
...
char **errmsg;
{
 ...
 chr  (char *)&word;
 for(tmp0; tmp<NBPW; tmp++, boff++, chr++) {
     if ( buffer[boff] ! *chr ) {
         sprintf(Errmsg, "Data mismatch at offset %d, exp:%#o, act:%#o",
...
printf("partial at end\n");
 ...
 for (tmp0; tmp<cnt && boff<bsize; boff++, tmp++, chr++) {
     if ( buffer[boff] ! *chr ) {
  sprintf(Errmsg, "Data mismatch at offset %d, exp:%#o, act:%#o",
      offset+boff, *chr, buffer[boff]);
  return offset+boff;
...
****/
    ...
    retdatapidchk(-1, buffer, size, 3, &errmsg);
    printf("datapidchk(-1, buffer, %d, 3, &errmsg) returned %d %s\n",
        size, ret, errmsg);
    retdatapidchk(-1, &buffer[1], size-1, 4, &errmsg);
...
******/
    ...
    retdatapidchk(getpid(), buffer, size, 5, &errmsg);
    printf("datapidchk(getpid(), buffer, %d, 5, &errmsg) returned %d %s\n",
        size, ret, errmsg);

    retdatapidchk(getpid(), &buffer[1], size-1, 6, &errmsg);
