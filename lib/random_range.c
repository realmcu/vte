...
random_bit(long mask)
{
     ...
     */
        ...
        bit1L;
        for ( nshift0; (unsigned int)nshift<sizeof(long)*8; nshift++) {
                if ( mask & bit )
                        nbits++;
                bitbit<<1;
        }
     ...
     */
    bitrandom_range(1, nbits, 1, NULL);
...
char **argv;
{
    int ind;
    int cnt, iter10;
    int imin0, imult1, itmin, itmax0;
#if CRAY
    int imax6*GIG; /* higher than 32 bits */
#else
    int imax1048576;
#endif

    long lret, lmin0, lmult1, ltmin, ltmax0;
#if CRAY || (_MIPS_SZLONG  64)
    long lmax6*(long)GIG; /* higher than 32 bits */
#else
    long lmax1048576;
#endif
    long long llret, llmin0, llmult1, lltmin, lltmax0;
 ...
 for(cnt0; cnt<iter; cnt++) {
     lretrandom_rangel(lmin, lmax, lmult, NULL);
     if ( iter < 100 )
         printf("%ld\n", lret);
     if ( lret < ltmin )
  ltmin  lret;
 ...
 * random_rangell() unit test
 ...
 for(cnt0; cnt<iter; cnt++) {
     llretrandom_rangell(llmin, llmax, llmult, NULL);
     if ( iter < 100 )
 ...
 for(cnt0; cnt<iter; cnt++) {
     lretrandom_range(imin, imax, imult, NULL);
     if ( iter < 100 )
         printf("%ld\n", lret);
            if ( lret < itmin )
                itmin  lret;
