...
int offset;
{
 ...
 if ( listofchars  NULL ) {
     charlistCHARS;
     chars_sizeCHARS_SIZE;
 }
 else {
     charlistlistofchars;
     chars_sizestrlen(listofchars);
 }

 for(cntoffset; cnt<total;  cnt++) {
  indcnt%chars_size;
  *chr++charlist[ind];
 }
...
char **errmsg;
{
 ...
 for(cntoffset; cnt<total;  chr++, cnt++) {
     indcnt%chars_size;
     if ( *chr ! charlist[ind] ) {
  sprintf(Errmsg,
...
char *errmsg;
    ...
    retdataasciichk(NULL, buffer, size, 0, &errmsg);
    ...
    retdataasciichk(NULL, &buffer[1], size-1, 1, &errmsg);
 ...
 printf("\tFAIL return value is %d, expected 25\n", ret);
    ...
    retdataasciichk("this is a test of the my string", buffer, size, 0, &errmsg);
    ...
    retdataasciichk("this is a test of the my string", &buffer[1], size-1, 1, &errmsg);
