...
int STD_FUNCTIONAL_TEST1, /* flag indicating to do functional testing code */
    STD_TIMING_ON0,  /* flag indicating to print timing stats */
    STD_PAUSE0,  /* flag indicating to pause before actual start, */
    /* for contention mode */
    STD_INFINITE0,  /* flag indciating to loop forever */
    STD_LOOP_COUNT1,  /* number of iterations */
    STD_COPIES1,  /* number of copies */
    STD_ERRNO_LOG0;  /* flag indicating to do errno logging */

float STD_LOOP_DURATION0.0,    /* duration value in fractional seconds */
      STD_LOOP_DELAY0.0; /* loop delay value in fractional seconds */


char **STD_opt_arr  NULL; /* array of option strings */
int    STD_nopts0,  /* number of elements in STD_opt_arr */
       STD_argind1;  /* argv index to next argv element */
    /* (first argument) */
 ...
 */
static int  STD_TP_barrier0; /* flag to do barrier in TEST_PAUSE */
    /* 2 - wait_barrier(), 3 - set_barrier(), * - barrier() */
static int  STD_LP_barrier0; /* flag to do barrier in TEST_LOOPING */
    /* 2 - wait_barrier(), 3 - set_barrier(), * - barrier() */
static int  STD_TP_shmem_sz0; /* shmalloc this many words per pe in TEST_PAUSE */
static int  STD_LD_shmem0; /* flag to do shmem_puts and shmem_gets during delay */
static int  STD_LP_shmem0; /* flag to do shmem_puts and gets during TEST_LOOPING */
static int  STD_LD_recfun0; /* do recressive function calls in loop delay */
static int  STD_LP_recfun0; /* do recressive function calls in TEST_LOOPING */
static int  STD_TP_sbrk0; /* do sbrk in TEST_PAUSE */
static int  STD_LP_sbrk0; /* do sbrk in TEST_LOOPING */
static char *STD_start_break0; /* original sbrk size */
static int  Debug0;
...
static struct usc_bigstack_t *STD_bigstackNULL;
...
parse_opts(int ac, char **av, option_t *user_optarr, void (*uhf)())
{
   ...
   /* found in the user's list */
    ...
    int options0; /* no options specified */
    int optstrlen, i;
    char *optionstr;
 ...
 switch (opt) {
  ...
  case 'I': /* Time duration */
   ...
   if ( STD_LOOP_DURATION  0.0 ) STD_INFINITE1;
   break;
  case 'c': /* Copies */
   options | OPT_copies;
  ...
  if (opt  user_optarr[i].option[0]) {
     ...
     * Turn on debug
     */
    if ( (ptrgetenv("USC_DEBUG")) ! NULL ) {
 Debug1;
        printf("env USC_DEBUG is defined, turning on debug\n");
    }
    if ( (ptrgetenv("USC_VERBOSE")) ! NULL ) {
 Debug1;
     ...
     */
    if ( !(options & OPT_iteration) && (ptrgetenv(USC_ITERATION_ENV)) ! NULL ) {
        if ( sscanf(ptr, "%i", &k)  1) {
  ...
  if ( Debug )
   ...
   USC_ITERATION_ENV, k);
     ...
     */
    if ( !(options & OPT_nofunccheck) && (ptrgetenv(USC_NO_FUNC_CHECK)) ! NULL ) {
        STD_FUNCTIONAL_TEST0; /* Turn off functional testing */
 if ( Debug )
  ...
  USC_NO_FUNC_CHECK);
    ...
    if ( !(options & OPT_duration) && (ptrgetenv(USC_LOOP_WALLTIME)) ! NULL ) {
        if ( sscanf(ptr, "%f", &ftmp)  1 && ftmp > 0.0 ) {
     STD_LOOP_DURATIONftmp;
     if ( Debug )
    ...
    if ( !(options & OPT_duration) && (ptrgetenv("USC_DURATION")) ! NULL ) {
        if ( sscanf(ptr, "%f", &ftmp)  1 && ftmp > 0.0 ) {
     STD_LOOP_DURATIONftmp;
     if ( Debug )
     ...
     */
    if ( !(options & OPT_delay) && (ptrgetenv(USC_LOOP_DELAY)) ! NULL ) {
        if ( sscanf(ptr, "%f", &ftmp)  1 && ftmp > 0.0 ) {
     STD_LOOP_DELAYftmp;
     if ( Debug )
  printf("Using env %s, set STD_LOOP_DELAY  %f\n",
     ...
     * The -c option with arg will be used even if this env var is set.
     */
    if ( !(options & OPT_copies) && (ptrgetenv(USC_COPIES)) ! NULL ) {
  ...
  printf("Using env %s, set STD_COPIES  %d\n",
     ...
     * hooks in the code.
     */
    if ( (ptrgetenv("USC_TP_BARRIER")) ! NULL ) {
 ...
 if ( Debug )
    ...
    if ( (ptrgetenv("USC_LP_BARRIER")) ! NULL ) {
        if ( sscanf(ptr, "%i", &k)  1 && k > 0 ) {
     STD_LP_barrierk;
 }
        else
 ...
 if ( Debug )
    ...
    if ( (ptrgetenv("USC_TP_SHMEM")) ! NULL ) {
        if ( sscanf(ptr, "%i", &k)  1 && k > 0 ) {
            STD_TP_shmem_szk;
     if ( Debug )
    ...
    if ( (ptrgetenv("USC_LP_SHMEM")) ! NULL ) {
        if ( sscanf(ptr, "%i", &k)  1 && k > 0 ) {
            STD_LP_shmemk;
     if ( Debug )
    ...
    if ( (ptrgetenv("USC_LD_SHMEM")) ! NULL ) {
        if ( sscanf(ptr, "%i", &k)  1 && k > 0 ) {
            STD_LD_shmemk;
     if ( Debug )
    ...
    if ( (ptrgetenv("USC_LP_RECFUN")) ! NULL ) {
        if ( sscanf(ptr, "%i", &k)  1 && k > 0 ) {
     STD_LP_recfunk;
  ...
  STD_bigstack(struct usc_bigstack_t *)
   malloc(sizeof(struct usc_bigstack_t));
    ...
    if ( (ptrgetenv("USC_LD_RECFUN")) ! NULL ) {
        if ( sscanf(ptr, "%i", &k)  1 && k > 0 ) {
     STD_LD_recfunk;
     if ( STD_bigstack ! (struct usc_bigstack_t *)NULL )
  STD_bigstack(struct usc_bigstack_t *)
...
usc_global_setup_hook()
{
  ...
  break;
     case 0:  /* child */
         cntSTD_COPIES;   /* to stop the forking */
  break;
 ...
 STD_start_breaksbrk(0); /* get original sbreak size */
    }
...
int counter;
{
    ...
    int hertz0;   /* clocks per second or usecs per second */
    int ct, end;  /* current time, end delay time */
    int keepgoing0;  /* used to determine return value */
