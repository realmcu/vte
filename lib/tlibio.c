...
lio_parse_io_arg1(char *string)
{
     ...
     */
    for(ind0; ind<sizeof(Lio_info1)/sizeof(struct lio_info_type); ind++) {
...
lio_parse_io_arg2(char *string, char **badtoken)
{
  ...
  * so, add the bits to the mask.
          */
        for(ind0; ind<sizeof(Lio_info2)/sizeof(struct lio_info_type); ind++) {
 ...
 *errmsg  Errormsg;
    ...
    Rec_signalReceived_signal; /* get the current number of signals received */
#ifdef sgi
    Rec_callbackReceived_callback; /* get the current number of callbacks received */
#endif
...
/*    aiocbp.aio_offset  lseek( fd, 0, SEEK_CUR ); -- set below */
 ...
 retlio_check_asyncio(io_type, size,  &status);
 return ret;
 ...
 retlio_check_asyncio(io_type, size,  &aiocbp, method);
 return ret;
#endif
    } /* LIO_IO_SLISTIO */
 ...
 return -1;
     ...
     */
#ifdef CRAY
    retlio_wait4asyncio(method, fd, statptr);
#endif
#ifdef sgi
    retlio_wait4asyncio(method, fd, &aiocbp);
#endif
...
#endif
{
 ...
 if( (ret  -1) && (errno  EINTR) ){
  if( aiocbp->aio_sigevent.sigev_notify  SIGEV_SIGNAL ){
   if( Debug_level > 2 ){
    printf("DEBUG %s/%d: aio_suspend received EINTR, sigev_notifySIGEV_SIGNAL -- ok\n",
           __FILE__, __LINE__ );
   }
  }
  else {
 ...
 * Just define add "-DUNIT_TEST1" to the cc line.
 *
 * (rrl 04/96)
 ***********************************************************************/
struct unit_info_t {
...
char **argv;
{
 ...
 case 'i': iter  atoi(optarg); break;
    ...
    if ((fdopen("unit_test_file", O_CREAT|O_RDWR|O_TRUNC, 0777))  -1 ) {
 ...
 exit(1);
    ...
    if ( symbols ! NULL ) {
        if ( (methodlio_parse_io_arg2(symbols,  &err))  -1 ){
 ...
 exit_status  0;
  ...
  ++exit_status;
          ...
          if ((retlio_write_buffer(fd, method, buffer,
  ...
  ++exit_status;
          ...
          if ((retlio_read_buffer(fd, method, buffer,
 ...
 exit(0);
    ...
    for(ind0; ind < sizeof(Unit_info)/sizeof(struct unit_info_t); ind++ ) {
 ...
 memset( buffer, 'A', 4096 );
        if ((retlio_write_buffer(fd, Unit_info[ind].method, buffer,
 ...
 memset( buffer, 'B', 4096 );
        if ((retlio_read_buffer(fd, Unit_info[ind].method, buffer,
   size, Unit_info[ind].sig, &err, 0)) ! size ) {
