 ...
 */
char    *keepalive = "%d Stats: mode %i, blocks %l, %m Mbytes, pass %p/%P,"
                      " elapsed %t";
                                        /* Default keepalive message.   */
char    *pkeepalive = "pass %p/%P, %l blocks, %m Mbytes, %r records,"
                      " errors %e/%E, elapsed %t";
                                        /* Per pass keepalive message.  */
char    *tkeepalive = "Totals for %d: %L blocks, %M Mbytes,"
	...
	if (alarmtime || runtime) {
            if (alarmtime) {
                if ( !keepalive || !strlen(keepalive) ) {
                    LogMsg (efp, logLevelCrit, 0,
                            "Please specify a keepalive string!\n");
                    exit (FATAL_ERROR);
                }
	...
	size_t		bytes )
{
    ...
    if (lba != NO_LBA) {
        ...
        if (boff && eoff) {
            bp += sprintf(bp, "%u.%u - %u.%u", lba, boff, elba, eoff);
        } else if (boff) {
            bp += sprintf(bp, "%u.%u - %u", lba, boff, elba);
        } else if (eoff) {
            bp += sprintf(bp, "%u - %u.%u", lba, elba, eoff);
        } else if (lba != elba) {
	    bp += sprintf(bp, "%u - %u", lba, elba);
        } else {
            bp += sprintf(bp, "%u", lba);
        }
    }
