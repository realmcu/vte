...
/**
@file nbamr_decoder_main.c

@brief VTE C main source file of the NB AMR decoder test application.
...
Test Assertion
& Strategy:  The NB AMR (Narrow band Adaptive Multi-Rate) decoder test applica-
             tion performs several tests. Some of them are required by the
             decoder documentation. The application itself consists of five
             test cases, which are selected via parameter -T <test_id>.
            ...
            Names of input, output and reference files are taken from file
            lists, located in ./list folder. Default file list is named
            def_test_files. Test case 3 has its own file list,
            robustness_test_files. Only input file names are mandatory; if
            output and reference files aren't required, their names can be
            replaced by n/a. Please see ./list folder for examples.

            All tests have common sequence of actions, described in detail in
            the documentation. For each test (input) file the application allo-
