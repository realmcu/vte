...
int run_decoder (gif_decoder_thread_t *enc_thread)
{
        ...
        int rv = TFAIL;
        int index, memreq;
        int glob_mem_index=0;	
        int mem_out_pix;
        char imf[MAX_STR_LEN];
        int mult_factor=3, scale_flag = FALSE;	
        int counter = 0;
        ...
        res=GIF_query_dec_mem (enc_thread->gif_dec_obj);
        ...
        for( memreq=0; memreq<enc_thread->gif_dec_obj->mem_info.num_reqs; memreq++ )
         {
            ...
            {      /* Check for priority and memory description can be added here */
              ...
              if(enc_thread->gif_dec_obj->mem_info.mem_info_sub[memreq].ptr==NULL)
              {
        ...
        glob_mem_index=enc_thread->gif_dec_obj->mem_info.num_reqs;
      ...
      /*Seek to the start of the file befor calling init*/
        ...
        /*Query for memory requirements of a frame*/
      		res=GIF_query_dec_mem_frame(enc_thread->gif_dec_obj);
	    ...
	    for(memreq=glob_mem_index;memreq<enc_thread->gif_dec_obj->mem_info.num_reqs;memreq++)
            {
             	if (enc_thread->gif_dec_obj->mem_info.mem_info_sub[memreq].type == E_FAST_MEMORY)
             	      	enc_thread->gif_dec_obj->mem_info.mem_info_sub[memreq].ptr
                                =alloc_fast(enc_thread->gif_dec_obj->mem_info.mem_info_sub[memreq].size);
		...
		res=GIF_decoder_init_frame(enc_thread->gif_dec_obj);
		...
		{
    			case E_OUTPUTFORMAT_RGB555:
        	 		 mult_factor=2; strcpy(enc_thread->node->rgb_str,"RGB555");
       	    		 break;
        	
              	case E_OUTPUTFORMAT_RGB565:
       			mult_factor=2; strcpy(enc_thread->node->rgb_str,"RGB565");
                		break;
        	
    	  		case E_OUTPUTFORMAT_RGB666:
        			mult_factor=4; strcpy(enc_thread->node->rgb_str,"RGB666");
				break;

			case E_OUTPUTFORMAT_RGB888:
        			mult_factor=3; strcpy(enc_thread->node->rgb_str,"RGB888");
				break;
...
#endif	      
	      if(enc_thread->out_fstream==NULL)
   	      {
...
int reentrance_test(flist_t * node)
{
    ...
    for ( i=0,node_n = node;  (node_n!=NULL)   &&  (i< MAX_CODEC_THREADS)  ;  i++, node_n  = node_n ->next)
    {
