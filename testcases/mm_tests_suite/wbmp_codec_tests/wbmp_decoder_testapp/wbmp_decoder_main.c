...
/* Global Variables */
char *TCID      "wbmp_decoder_testapp"; /* test program identifier.          */
...
void cleanup()
{
    ...
    VT_rv  VT_wbmp_decoder_cleanup();
    if( VT_rv ! TPASS )
    {
 tst_resm( TWARN, "VT_wbmp_decoder_cleanup() Failed : error code  %d", VT_rv );
    }
...
void setup()
{
    ...
    VT_rv  VT_wbmp_decoder_setup();
    if( VT_rv ! TPASS )
    {
 tst_brkm( TBROK , cleanup, "VT_wbmp_decoder_setup() Failed : error code  %d", VT_rv );
    }
    /* VTE */
...
int main( int argc, char ** argv )
{
 ...
 cfg_file  cfg_opt;
    ...
    VT_rv  VT_wbmp_decoder_test( testcase, iter, cfg_file ); /* with the parameters needed come from parse_opt()) */
