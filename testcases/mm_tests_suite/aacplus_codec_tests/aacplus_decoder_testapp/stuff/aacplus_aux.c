...
*                           parse headers.
*   29/08/2005     02      Removed aacd_tables.h header        Ashok Kumar
*                          file
******************************************************************************/
#include <string.h>

#include "../aacplus_decoder_test.h"
#include "aacplus_aux.h"
#include "aacplus_misc.h"

#define App_adif_header_present pBitstream->mAppAdifHeaderPresent
#define App_adts_header_present pBitstream->mAppAdtsHeaderPresent

#define bitstream_buf           pBitstream->mpBitstreamBuf
#define bitstream_buf_index     pBitstream->mBitstreamBufIndex
#define bitstream_count         pBitstream->mBitstreamCount
#define in_buf_done             pBitstream->mInBufDone
#define bytes_supplied          pBitstream->mBytesSupplied

#define BitsInHeader            pBitstream->mBitsInHeader

#define App_ibs_buf             pBitstream->mAppIbsBuf
#define App_bs_param            pBitstream->mAppBsParam
...
int App_bs_refill_buffer (sBitstream * pBitstream)
{
        ...
        if (bytes_to_copy < 0)
        {
                ...
                else
                {
                        ...
                        b->bs_curr_ext  (unsigned char *)(bitstream_buf + bitstream_buf_index);
...
void App_bs_readinit(sBitstream * pBitstream, char *buf, int bytes)
{
        ...
        b->bs_curr_ext  (unsigned char *)buf;
        ...
        while (b->bit_counter > 0)
        {
                ...
                {
                        ret  App_bs_refill_buffer(pBitstream);
                        if (ret < 0)
                                break;
                }
                temp  *b->bs_curr++;
...
*       None
*******************************************************************************/
static void App_get_ele_list(sBitstream * pBitstream, AACPD_EleList * p, int enable_cpe)
{
        int             i,
                j;
        ...
        {
                if (enable_cpe)
                        p->ele_is_cpe[i]  App_bs_read_bits(pBitstream,LEN_ELE_IS_CPE);
                else
                        p->ele_is_cpe[i]  0;
                p->ele_tag[i]  App_bs_read_bits(pBitstream, LEN_TAG);
        }
}
...
int App_get_prog_config(sBitstream * pBitstream, AACPD_ProgConfig * p)
{
        ...
        p->tag  App_bs_read_bits(pBitstream, LEN_TAG);

        p->profile  App_bs_read_bits(pBitstream, LEN_PROFILE);
        if (p->profile ! 1)
        {
                return -1;
        }
        p->sampling_rate_idx  App_bs_read_bits(pBitstream, LEN_SAMP_IDX);
        if (p->sampling_rate_idx > 0xc)
        {
                return -1;
        }
        p->front.num_ele  App_bs_read_bits(pBitstream, LEN_NUM_ELE);
        if (p->front.num_ele > FChans)
        {
                return -1;
        }
        p->side.num_ele  App_bs_read_bits(pBitstream, LEN_NUM_ELE);
        if (p->side.num_ele > SChans)
        {
                return -1;
        }
        p->back.num_ele  App_bs_read_bits(pBitstream, LEN_NUM_ELE);
        if (p->back.num_ele > BChans)
        {
                return -1;
        }
        p->lfe.num_ele  App_bs_read_bits(pBitstream, LEN_NUM_LFE);
        if (p->lfe.num_ele > LChans)
        {
                return -1;
        }
        p->data.num_ele  App_bs_read_bits(pBitstream, LEN_NUM_DAT);
        p->coupling.num_ele  App_bs_read_bits(pBitstream, LEN_NUM_CCE);
        if (p->coupling.num_ele > CChans)
        {
                return -1;
        }
        if ((p->mono_mix.present  App_bs_read_bits(pBitstream, LEN_MIX_PRES))  1)
                p->mono_mix.ele_tag  App_bs_read_bits(pBitstream, LEN_TAG);
        if ((p->stereo_mix.present  App_bs_read_bits(pBitstream, LEN_MIX_PRES))  1)
                p->stereo_mix.ele_tag  App_bs_read_bits(pBitstream, LEN_TAG);
        if ((p->matrix_mix.present  App_bs_read_bits(pBitstream, LEN_MIX_PRES))  1)
        {
                p->matrix_mix.ele_tag  App_bs_read_bits(pBitstream, LEN_MMIX_IDX);
                p->matrix_mix.pseudo_enab  App_bs_read_bits(pBitstream, LEN_PSUR_ENAB);
        }
        App_get_ele_list(pBitstream, &p->front, 1);
        App_get_ele_list(pBitstream, &p->side, 1);
        App_get_ele_list(pBitstream, &p->back, 1);
        App_get_ele_list(pBitstream, &p->lfe, 0);
        App_get_ele_list(pBitstream, &p->data, 0);
        App_get_ele_list(pBitstream, &p->coupling, 1);

        App_bs_byte_align(pBitstream);

        j  App_bs_read_bits(pBitstream, LEN_COMMENT_BYTES);
        ...
        */
        ...
        for (i  0; i < j; i++)
                p->comments[0]  App_bs_read_bits(pBitstream, LEN_BYTE);
...
int App_get_adif_header(sBitstream * pBitstream, AACPD_Block_Params * params)
{
        ...
        /* copyright string */
        if ((p->copy_id_present  App_bs_read_bits(pBitstream, LEN_COPYRT_PRES))  1)
        {
                for (i  0; i < LEN_COPYRT_ID; i++)
                        p->copy_id[i]  App_bs_read_bits(pBitstream, LEN_BYTE);

                /* null terminated string */
                p->copy_id[i]  0;
        }
        p->original_copy  App_bs_read_bits(pBitstream, LEN_ORIG);
        p->home  App_bs_read_bits(pBitstream, LEN_HOME);
        p->bitstream_type  App_bs_read_bits(pBitstream, LEN_BS_TYPE);
        p->bitrate  App_bs_read_bits(pBitstream, LEN_BIT_RATE);

        /* program config elements */
        select_status  -1;
        n  App_bs_read_bits(pBitstream, LEN_NUM_PCE) + 1;

        tmp_config  (AACPD_ProgConfig*) malloc (n*sizeof(AACPD_ProgConfig));

        for (i  0; i < n; i++)
        {
                tmp_config[i].buffer_fullness 
                        (p->bitstream_type  0) ? App_bs_read_bits(pBitstream, LEN_ADIF_BF) : 0;


                if (App_get_prog_config(pBitstream, &tmp_config[i]))
                {
                        free( tmp_config );
                        return -1;
                }
        ...
        /* Fill in the AACPD_Block_Params struct now */
...
int App_get_adts_header(sBitstream * pBitstream, AACPD_Block_Params * params)
{
        ...
        * the ADTS header, then something is wrong, so exit.
                */
                ...
                bits_used + App_bs_byte_align(pBitstream);
                ...
                p->syncword  App_bs_read_bits (pBitstream, (LEN_SYNCWORD - LEN_BYTE));
                bits_used + LEN_SYNCWORD - LEN_BYTE;

                /* Search for syncword */
                while (p->syncword ! ((1 << LEN_SYNCWORD) - 1))
                {
                        p->syncword  ((p->syncword << LEN_BYTE) | App_bs_read_bits(pBitstream, LEN_BYTE));
                ...
                p->id  App_bs_read_bits(pBitstream, LEN_ID);
                bits_used + LEN_ID;
                ...
                */
                ...
                p->layer  App_bs_read_bits(pBitstream, LEN_LAYER);
                bits_used + LEN_LAYER;
                if (p->layer ! 0)
                {
                        continue;
                ...
                p->protection_abs  App_bs_read_bits(pBitstream, LEN_PROTECT_ABS);
                ...
                p->sampling_freq_idx  App_bs_read_bits(pBitstream, LEN_SAMP_IDX);
                ...
                p->private_bit  App_bs_read_bits(pBitstream, LEN_PRIVTE_BIT);
                bits_used + LEN_PRIVTE_BIT;

                //temp_channel_config  p->channel_config;
                p->channel_config  App_bs_read_bits(pBitstream, LEN_CHANNEL_CONF);
                ...
                if (p->id  0)
                {
                        emphasis  App_bs_read_bits(pBitstream, LEN_EMPHASIS);
                ...
                p->copyright_id_bit  App_bs_read_bits(pBitstream, LEN_COPYRT_ID_ADTS);
                bits_used + LEN_COPYRT_ID_ADTS;

                p->copyright_id_start  App_bs_read_bits(pBitstream, LEN_COPYRT_START);
                bits_used + LEN_COPYRT_START;

                p->frame_length  App_bs_read_bits(pBitstream, LEN_FRAME_LEN);
                bits_used + LEN_FRAME_LEN;

                p->adts_buffer_fullness  App_bs_read_bits(pBitstream, LEN_ADTS_BUF_FULLNESS);
                bits_used + LEN_ADTS_BUF_FULLNESS;

                p->num_of_rdb  App_bs_read_bits(pBitstream, LEN_NUM_OF_RDB);
                ...
                */
                ...
                {
                        p->crc_check  App_bs_read_bits(pBitstream, LEN_CRC);
                        bits_used + LEN_CRC;
                }
