...
-------------------------   ------------    ----------  -------------------------------------------
D.Simakov / smkd001c        12/10/2004      TLSbo43522   Intial version
...
/* Global Variables */
char *TCID      "g726_decoder_testapp"; /* test program identifier.          */
...
void cleanup()
{
    ...
    VT_rv  VT_g726_decoder_cleanup();
    if( VT_rv ! TPASS )
    {
        tst_resm( TWARN, "VT_g726_decoder_cleanup() Failed : error code  %d", VT_rv );
    }
...
void setup()
{
    ...
    VT_rv  VT_g726_decoder_setup();
    if( VT_rv ! TPASS )
    {
        tst_brkm( TBROK , cleanup, "VT_g726_decoder_setup() Failed : error code  %d", VT_rv );
    }
    /* VTE */
...
int main( int argc, char ** argv )
{
    ...
    {
        tst_brkm( TBROK, cleanup, "Argument required -C", msg );
    }
    ...
    VT_rv  VT_g726_decoder_test( testcase, iter, cfg_file ); /* with the parameters needed come from parse_opt()) */
