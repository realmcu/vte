...
/* Global Variables */
char *TCID      "g726_encoder_testapp"; /* test program identifier.          */
...
void cleanup()
{
    ...
    VT_rv  VT_g726_encoder_cleanup();
    if( VT_rv ! TPASS )
    {
        tst_resm( TWARN, "VT_g726_encoder_cleanup() Failed : error code  %d", VT_rv );
    }
...
void setup()
{
    ...
    VT_rv  VT_g726_encoder_setup();
    if( VT_rv ! TPASS )
    {
        tst_brkm( TBROK , cleanup, "VT_g726_encoder_setup() Failed : error code  %d", VT_rv );
    }
    /* VTE */
...
int main( int argc, char ** argv )
{
    ...
    VT_rv  VT_g726_encoder_test( testcase, iter, cfg_file ); /* with the parameters needed come from parse_opt()) */
