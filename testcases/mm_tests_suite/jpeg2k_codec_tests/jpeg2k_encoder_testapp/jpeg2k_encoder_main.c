...
void cleanup()
{
    ...
    rv = VT_jpeg2k_encoder_cleanup();
    if( rv != TPASS )
    {
	    tst_resm( TWARN, "VT_jpeg2k_encoder_cleanup() Failed : error code = %d", rv );
    }
...
void setup()
{
    ...
    rv = VT_jpeg2k_encoder_setup();
    if( rv != TPASS )
    {
    	tst_brkm( TBROK , cleanup, "VT_jpeg2k_encoder_setup() Failed : error code = %d", rv );
    }
