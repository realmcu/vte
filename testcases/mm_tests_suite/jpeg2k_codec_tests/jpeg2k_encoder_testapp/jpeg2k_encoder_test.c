...
#define SAFE_DELETE(p) {if(p){Util_Free(p);p=NULL;}}
#define NA "n/a"
#define M(m){printf("<<<--- %s --->>>\n",m);fflush(stdout);}
                ...
                if( pHandler->mCompPsnr[i] )
                {
                    if( gTestappConfig.mVerbose )
                    {
                        TST_RESM( TINFO, "Thread[%lu] %c PSNR is %4.4f", 
                                  pHandler->mIndex,  
                                  i==0 ? 'Y' : (i==1 ? 'U' : 'V'), pHandler->mCompPsnr[i] );
                    }                                  
                }
                else
                {
...
{    
    ...
    {   
        ...
        {
            FILE * pOutStream;
            sprintf( pHandler->mDecodedFileName[i], "%s_dec_%c.raw", outFileName, (i==0?'y':(i==1)?'u':'v') );
