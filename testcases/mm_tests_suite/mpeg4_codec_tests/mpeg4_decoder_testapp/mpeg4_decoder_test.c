...
int allocate_memory(sMpeg4DecMemAllocInfo *psMemInfo)
{
    ...
    for(i0;i<psMemInfo->s32NumReqs;i++)
    {
...
int decoder_engine( void * ptr )
{
 ...
 return ret;
    ...
    sMpeg4DecObj.sVisParam.s32NumberOfVos1;
...
int run_decoder_in_loop( void * ptr )
{
    ...
    {
              if((test_testcaseRE_ENTRANCE)||(test_testcasePRE_EMPTION))
                                    ...
                                    void*  ptr)
{
     ...
     while (1) /* Decode Loop */
     {
          ...
          {
             if(s32FrameNum+1test_start_frame) /* begining to skip */
             {
#ifdef DEBUG_TEST
   ...
   s32SrcFrameNum  psMpeg4DecObject->sDecParam.sTime.s32Seconds *
     ...
     if(test_testcaseADVANCED_FUNCTIONALITY)
  ...
  if(pDecThread->frame)
  {
   ...
   if( offset_bpp < gVideoBufferSize )
   {
     UCHAR * ptr_frame  pDecThread->frame + offset_bpp;
           ...
           if(rr<0) rr  0; if(rr>255) rr255;
           if(gg<0) gg  0; if(gg>255) gg255;
           if(bb<0) bb  0; if(bb>255) bb255;
           rrr;ggg;bbb;
/*
