...
/*# #define GRS_INCLUDED */  /* for greyscale pics     */
...
/*
#define GRS_FILE(i) greyscale_##i
#define RGB_FILE(i)     rgb565_##i
*/
...
PFunc r_func  oplIPRotate_CW_U16, R_func  oplIPRotate_CCW_U16;
...
int test_rotate_clwise (const int, const int);
int test_rotate_cclwise(const int, const int);
int test_rotate(const int srcBuffW, const int srcBuffH, int op);
...
int VT_opl_rotate_setup()
{
    ...
    case 1:
        type  GRS;
        r_func  oplIPRotate_CW_U8;
        R_func  oplIPRotate_CCW_U8;
        printf("\n set 8 ");
        break;
    case 2:
        type  RGB565;
        r_func  oplIPRotate_CW_U16;
        R_func  oplIPRotate_CCW_U16;
        break;
...
*/
/**/
int VT_opl_rotate_cleanup(void)
{
    int rv  TFAIL;
...
*/
/**/
int VT_opl_rotate_test(int Id, int Case, int Iter, int Write_flag, int r_flag, int R_flag)
{
    ...
    switch (Id)
    {
     ...
     if (Write_flag)
  GenerateBitmap(src, type, HEIGHT, WIDTH, "orig_pic_r.bmp");

  if(r_flag)
        {
                /* rotating picture clockwise */



             if ((errr_func(src, rect, linestride, dst)) ! OPLErrorSuccess)
             {
                fprintf(stderr, "\n\tError: rotation clockwise failed\n");
              print_err_type(err);
             ...
             rect.mWidth   HEIGHT; /* pictire is rotated */
             rect.mHeight  WIDTH;

             draw_pic(dst, type, rect);

      if (Write_flag)
   GenerateBitmap(dst, type, WIDTH, HEIGHT, "rot_cl_pic.bmp");

        }

        if(R_flag)
        {
            /* rotating picture counterclockwise */
            rect.mWidth   WIDTH; /* pictire is rotated */
            rect.mHeight  HEIGHT;

            if ((err  R_func(src, rect, linestride, dst)) ! OPLErrorSuccess)
         {
          fprintf(stderr, "\n\tError: rotation counterclockwise failed\n");
          print_err_type(err);
         }

         printf("Rotated counterclockwise...\n\n");
     ...
     if (Write_flag)
  GenerateBitmap(dst, type, WIDTH, HEIGHT, "rot_ccl_pic.bmp");
        }
    ...
    case RESTITUTION: /* verify that twice mirrored picture is the same picture*/
     ...
     memcpy(ref_src, src, WIDTH*HEIGHT*bytes_per_pixel);        /* create reference copy for bit comparing */
        ...
        while(r_flag | R_flag )
        {
             ...
             memcpy(src, ref_src, WIDTH*HEIGHT*bytes_per_pixel);
                linestride  WIDTH*bytes_per_pixel;

                memset(dst,0,WIDTH*HEIGHT*bytes_per_pixel);
             if ((err  func(src, rect, linestride, dst)) ! OPLErrorSuccess)
             {
                 fprintf(stderr, "\n\tError: first rotation failed\n");
                 print_err_type(err);
                 err_count++;
             }
             ...
             linestride  HEIGHT*bytes_per_pixel;
             rect.mWidth   HEIGHT;      /* pictire is rotated */
             rect.mHeight  WIDTH;
             ...
             printf("First step...\n\n");
                ...
                memset(src,0,WIDTH*HEIGHT*bytes_per_pixel);
             if ((err  func(dst, rect, linestride, src)) ! OPLErrorSuccess)
             {
                 fprintf(stderr, "\n\tError: second rotation failed\n");
                 print_err_type(err);
                 err_count++;
             }
...
void* thread_test_func(void* ptr)
{
    ...
    rv1  test_rotate_clwise(x_size,y_size);
    rv2  test_rotate_cclwise(x_size,y_size);
...
int test_rotate_clwise(const int srcBuffW, const int srcBuffH)
{
    return test_rotate(srcBuffW, srcBuffH, 'r');
}

int test_rotate_cclwise(const int srcBuffW, const int srcBuffH)
{
    return test_rotate(srcBuffW, srcBuffH,'R');
}
...
int test_rotate(const int srcBuffW, const int srcBuffH, int op)
{
    ...
    const int   dstBuffW         srcBuffH;
    const int   dstBuffH         srcBuffW;
    ...
    dstRect.mX           0;
    dstRect.mY           0;
    dstRect.mWidth       srcBuffH;
    dstRect.mHeight      srcBuffW;
   ...
   dst  (U8*)malloc(dstBuffBytes);
    ...
    /*
    for(i0;i<srcBuffW*srcBuffH*bytes_per_pixel;i++)
      p[i]  rand();
    */

   for(i0;i<bytes_per_pixel;i++)
    p[srcX*bytes_per_pixel+srcY*srcBuffW*bytes_per_pixel+i]  0xAA;
   ...
   if(verbose_mode)
   {
    ...
    for(i0;i<bytes_per_pixel;i++)
    {
        if (   p[dstX*bytes_per_pixel+dstY*dstBuffW*bytes_per_pixel+i] ! 0xAA)
         rv   TFAIL;
    }

    if( rv  TPASS)
    {
     printf("rotation works properly\n\n");
    }
    else
    {
     printf("rotation doesn't work properly\n\n");
    }
...
*/
    ...
    return file_size;
      ...
      /* type not used so far - default RGB565 */
