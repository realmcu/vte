...
void scenar_init()
{
 ...
 for (i0; i<NSCENAR; i++)
 {
  ...
  if (tps>0) /* This routine is dependent on the Thread Execution Scheduling option */
  {
   ...
   #if VERBOSE > 4
    output("Contension scope set to %s\n", oldPTHREAD_SCOPE_PROCESS?"PTHREAD_SCOPE_PROCESS":"PTHREAD_SCOPE_SYSTEM");
   }
   else
   {
...
int sc0; /* This might be very dirty... but is much simpler */
