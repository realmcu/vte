...
#define UNRESOLVED_KILLALL(error, text) { \
 ...
 { \
  int _nch; \
  for (_nch0; _nch<children.nb; _nch++) \
...
typedef struct
{
 intcount;     /* number of children currently waiting */
 pthread_cond_t cnd;
 pthread_mutex_t mtx;
 intpredicate; /* Boolean associated to the condvar */
 ...
 */
 ...
 for (scenar0; scenar < NSCENAR; scenar++)
 {
  ...
  td->mtypescenarii[scenar].m_type;
  ...
  for (children.nb0; children.nb<NCHILDREN; children.nb++)
  {
   if (td->fork0)
   {
