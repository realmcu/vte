...
main(int argc, char **argv)
{
 ...
 case 'i': fid  atoi(optarg);     /* set chassis id on/off */
    fsetid  1;
                  break;
 case 'o': fsetid0; fid0;   /* set all alarms off */
    for (j  0; j < NLEDS; j++) {
   leds[j].fset  1;
   leds[j].val  0;
    }
...
rptentry[0] resourceid1 tag: Mullins
RDR[45]: ctltype2:1 oem0  Chassis Identify Control
RDR[48]: ctltype0:1 oem10 Front Panel Power Alarm LED         state  off
RDR[51]: ctltype0:1 oem13 Front Panel Minor Alarm LED         state  ON
RDR[46]: ctltype0:0 oem0  Cold Reset Control
RDR[49]: ctltype0:1 oem11 Front Panel Critical Alarm LED      state  off
RDR[50]: ctltype0:1 oem12 Front Panel Major Alarm LED         state  off
 *-----------------------------------------------------------*/
/* end hpialarmpanel.c */
