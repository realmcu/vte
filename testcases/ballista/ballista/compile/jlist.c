 ...
 starting with an unsorted list... */
{
  ...
  if (theList->sort_ascending) m1;     /* ascending/descending multiplier*/
  else m-1;
 ...
 {
   temp1  theList->head;
   while ((temp1 !NULL)&&((theList->compare(theElement,temp1))*m1))
     ...
     /* this function returns a pointer to the next element in the list*/
{
  ...
  if (theList->headNULL) return (NULL);   /* Does the list exist*/
