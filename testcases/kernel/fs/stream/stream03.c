...
int main(int ac, char *av[])
{
 ...
 //block0:
  ...
  posftell(stream);
  if ( pos ! 0 ) {
   tst_resm(TFAIL,0,"file pointer descrepancy 3 (pos%zi, wanted pos0)", pos);
   local_flag  FAILED;
  }
  ...
  posftell(stream);
  if (pos ! 0 ) {
   tst_resm(TFAIL,"file pointer descrepancy 6 (pos%zi, wanted pos0)", pos);
   local_flag  FAILED;
  }
 ...
 //block1:
  ...
  oposftello(stream);
  if ( opos ! 0 ) {
   tst_resm(TFAIL,"file pointer descrepancy 1 (opos%zi, wanted opos0)", opos);
   local_flag  FAILED;
  }
  /* write something and check */
  if(fwrite(junk,sizeof(*junk),strlen(junk),stream)  0) {
  ...
  oposftello(stream);
  if ( opos ! 0 ) {
   tst_resm(TFAIL,"file pointer descrepancy 3 (opos%zi, wanted opos0)", opos);
   local_flag  FAILED;
  }
  /* seek from current position and then check */
  ...
  oposftello(stream);
  if (opos ! 0 ) {
   tst_resm(TFAIL,"file pointer descrepancy 6 (opos%zi, wanted opos0)", opos);
   local_flag  FAILED;
  }
