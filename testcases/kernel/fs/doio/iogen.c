...
int Orealtime0;  /* flag set for -O REALTIME      */
int Oextsize0;  /* real-time extent size      */
int Oreserve1;  /* flag for -O [no]reserve      */
int Oallocate0;  /* flag for -O allocate       */
int Owrite1;  /* flag for -O nowrite       */
...
struct io_req   *req;
{
    int         mult, offset0, length0, slength;
 ...
 req->r_data.read.r_aio_strat  (aio_stratNULL) ? 0 : aio_strat->m_value;
 ...
 req->r_data.write.r_aio_strat  (aio_stratNULL) ? 0 : aio_strat->m_value;
 ...
 req->r_data.io.r_aio_strat  (aio_stratNULL) ? 0 : aio_strat->m_value;
 ...
 req->r_data.listio.r_aio_strat  (aio_stratNULL) ? 0 : aio_strat->m_value;
...
int nbytes;
{
 ...
 if(Owrite  2) {
  ...
  buf + finfo.d_mem - ((long)buf % finfo.d_mem);
     ...
     if ( (rvalwrite(fd, buf, finfo.d_miniosz)) ! finfo.d_miniosz) {
  fprintf(stderr,
...
char *opts;
{
 ...
 case 'O':
  ...
  /* Oreserve1 by default; this clears that default */
  Oreserve0;
     } else if(!strcmp(openargs[0], "nowrite")) {
  /* Owrite1 by default; this clears that default */
  Owrite0;
