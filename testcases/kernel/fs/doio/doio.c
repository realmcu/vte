...
int rw;
{
 ...
 /* move to the desired file position. */
 if ((rclseek(fd, req->r_data.io.r_offset, SEEK_SET))  -1) {
  status->rval  rc;
  status->err  errno;
  return(status);
 }
...
fmt_mmrw(struct io_req *req, struct syscall_info *sy, int fd, char *addr)
{
 ...
 cp + sprintf(cp, "\tfile-mem0x%lx, length%d, buffer0x%lx\n",
 ...
 struct io_req *req;
{
  ...
  */
 for(sysyscalls; sy->sy_name ! NULL && sy->sy_type ! req->r_type; sy++)
  ;
 ...
 struct io_req *req;
{
 ...
 switch (req->r_type) {
 case RESVSP: opF_RESVSP; msg"f_resvsp";  break;
 case UNRESVSP: opF_UNRESVSP; msg"f_unresvsp"; break;
#ifdef F_FSYNC
 case DFFSYNC: opF_FSYNC; msg"f_fsync";  break;
#endif
 }
...
{
 havesigint1; /* in case there's a followup signal */
...
FILE *stream;
{
 ...
 fprintf(stream, "\t                         select:time (1 second1000000)\n");
 fprintf(stream, "\t                         sleep:time (1 second1)\n");
#ifdef sgi
 fprintf(stream, "\t                         sginap:time (1 secondCLK_TCK100)\n");
#endif
 fprintf(stream, "\t                         alarm:time (1 second1)\n");
