...
int max_size5000;
int min_size1;   /* also set in option parsing */
int mult_size1;  /* when random iosz, iosz must be mult of mult_size */
/* the *_lseek variables are only used when radon lseek option (-R) is used */
int min_lseek0;  /* also set in option parsing */
int max_lseek-1;  /* -1 means size of file */
#ifdef CRAY
int PatternPATTERN_OFFSET; /* This pattern is 64 bit word based */
#else
int PatternPATTERN_ASCII;
#endif
int Seed-1;   /* random number seed, < 0  uninitialized  */
int Nseeds0;   /* Number of seed specified by the user */
int *Seeds;   /* malloc'ed arrary of ints holding user spec seeds */

int using_random0;  /* flag indicating randomization is being used */
float delaysecs0.0;  /* delay between iterations (in seconds) */
int delaytime;   /* delay between iterations in clocks/uses */
int lockfile0;   /* if set, do file locking */
    /* 1  do file locking around write, trunc */
    /* and reads. */
    /* 2  write lock around all file operations */

off_t Woffset0;  /* offset before last write */
int Grow_incr4096;  /* sz of last write */
int Mode0;   /* bitmask of write/trunc mode */
    /* also knows if dealing with fifo */
char *Buffer  NULL;  /* buffer used by write and write check */
int Alignment0;  /* if non word multiple, io will not be word aligned */
int Opid0;   /* original pid */
...
int num_procs1;
int forker_mode0;
int reexecREXEC_INIT;  /* reexec info */
char *exec_pathNULL;
...
struct timeval tv1;
  ...
  */
 if ((Prognamestrrchr(argv[0], '/')) ! NULL)
  Progname++;
 else
  Prognameargv[0];
 ...
 while ((indgetopt(argc, argv,
  ...
  case 'g':
   if ((retsscanf(optarg, "%i%c", &grow_incr, &chr)) < 1 ||
    grow_incr < 0 ) {
  ...
  case 'I':
#if NEWIO
   if((io_typelio_parse_io_arg1(optarg))  -1 ) {
       fprintf(stderr,
   ...
   if( io_type & LIO_RANDOM )
    using_random++;
#else
   if((io_typeparse_io_arg(optarg))  -1 ) {
       fprintf(stderr,
    "%s%s: --I arg is invalid, must be s, p, f, a, l, L or r.\n",
    Progname, TagName);
       exit(1);
  ...
  case 'l':
   lockfile++;
   if ( lockfile > 2 )
      lockfile2; /* lockfile can only be 1 or 2 */
   break;
  ...
  case 'o':
       ...
       open_flagsRANDOM_OPEN;
       using_random++;

          } else if ((open_flagsparse_open_flags(optarg, NULL))  -1 ) {
  ...
  case 'R': /* random lseek before write arg: [min-]max*/
   if (sscanf(optarg, "%i-%i", &min_lseek, &max_lseek) ! 2 ) {
       min_lseek1;    /* same as default in define */
  ...
  case 's': /* format: seed[,seed...] */
   ...
   cptroptarg;
   for(Nseeds1; *cptr ; Nseeds++) {
       if ( (filenamestrchr(cptr, ','))  NULL )
    break;
       cptrfilename;
       cptr++;
   }
  ...
  case 't':
   if ((retsscanf(optarg, "%i%c", &trunc_incr, &chr)) < 1 ||
    trunc_incr < 0 ) {
...
no whole file checking will be performed!\n", Progname, TagName, getpid());
  ...
  */
  ...
  printf("%s: %d DEBUG3 unlink_inter  %d, unlink_inter_ran  %d\n",
     ...
     if ( Debug > 8 ) {
        numsizeof(Open_flags)/sizeof(int);
        printf("%s: %d DEBUG9 random open flags values:\n", Progname, Pid);
        for(ind0; ind<num; ind++) {
      printf("\t%#o\n", Open_flags[ind]);
        }
     }
 }  /* end of DEBUG > 2 */
 ...
 Pidgetpid(); /* reset after the forks */
 /*
 ...
 if ( Nseeds  1 ) {
  ...
  if ( Forker_npids > Nseeds )
  {
   ...
   Seedts.tv_sec + Pid;  /* default random seed */
  }
  else {
      SeedSeeds[Forker_npids];
  }
     }
 } else {
     /*
  ...
  gettimeofday(&ts,NULL);
     Seedts.tv_sec + Pid;  /* default random seed */
     //Seedtime(0) + Pid;  /* default random seed */
 ...
 if ( unlink_inter_ran > 0 ) {
      ...
      */
     tmpunlink_inter;
     unlink_interrandom_range(tmp, unlink_inter_ran, 1, NULL);
 ...
 num0;
 if ( first_file_ind ) {
  for(indfirst_file_ind; ind<argc; ind++, num++) {
 ...
 for(ind0;ind<num_auto_files; ind++, num++) {
  ...
  */
 for(ind1; ind<seq_auto_files; ind++, num++) {
...
/**** end filename stuff ****/
 ...
 if ( pre_alloc_space ) {
  ...
  total_grow_valuegrow_incr * iterations;

  /*
   * attempt to limit
   */
  if ( bytes_to_consume && bytes_to_consume < total_grow_value ) {
   total_grow_valuebytes_to_consume;
  }
 }
#endif
 ...
 for(Iter_cnt1; ! stop ; Iter_cnt++) {
  ...
  if ( open_flags   RANDOM_OPEN ) {
     retOpen_flags[random_range(0, sizeof(Open_flags)/sizeof(int)-1, 1, NULL)];
  }

  else
     retopen_flags;

  Fileinfo.openflagsret;
  ...
  if ( pre_alloc_space ) {
   ...
   close(fd);
   Iter_cnt0; /* reset outside loop to restart from one */
   continue;
  }
#endif
  ...
  if ( unlink_inter && (Iter_cnt % unlink_inter  0) ) {
   ...
   printf("%s: %d DEBUG5 %s/%d: %d Unlinking file %s\n",
      ...
      ctcurtime.tv_sec*USECS_PER_SEC + curtime.tv_usec;
      endct+delaytime;
                    while ( ct < end ) {
...
#if CRAY
 send_signals1; /* only send signals once */
 if ( Debug > 1 )
...
long *curr_size_ptr; /* BUG:14136 */
{
 ...
 if ( Mode & MODE_RAND_SIZE ) {
     grow_incrrandom_range(min_size, max_size, mult_size, &errmsg);
     if (errmsg ! NULL) {
 ...
 if ( ! (Mode & MODE_FIFO) ) {
     if ((cur_offsetlseek(fd,0,SEEK_CUR))  -1 ) {
 ...
 if ( Mode & MODE_GROW_BY_LSEEK ) {
  ...
  if ((noffsetlseek(fd, grow_incr-1, SEEK_END))  -1 ) {
...
#if NEWIO
  retlio_write_buffer(fd, io_type, "w", 1, SIGUSR1, &errmsg,0);
#else
  retwrite_buffer(fd, io_type, "w", 1, 0, &errmsg);
#endif
...
****/
  ...
  if ( Fileinfo.openflags & O_APPEND ) {
   ...
   printf("%s: %d DEBUG3 %s/%d: dealing with O_APPEND condition (offset:fsz:%d)\n",
       Progname, Pid, __FILE__, __LINE__, (int)stbuf.st_size);
                   ...
                   if ( max_lseek  LSK_EOF ) { /* within file size */
                        noffsetrandom_range(min_lseek, fsize, 1, NULL);
                   }
     else if ( max_lseek  LSK_EOFPLUSGROW ) {
          /* max to beyond file size */
   noffsetrandom_range(min_lseek, fsize+grow_incr, 1, NULL);
     }
     else if ( max_lseek  LSK_EOFMINUSGROW ) {
          /*
   ...
   if ( fsize < min_lseek + grow_incr )
       noffsetmin_lseek;  /* file will still grow */
   else
       noffsetrandom_range(min_lseek, fsize-grow_incr, 1, NULL);
     }
                   else {
                        noffsetrandom_range(min_lseek, max_lseek, 1, NULL);
                   }

     if ((Woffsetlseek(fd, noffset, SEEK_SET))  -1 ) {
   ...
   */
      ...
      if ((Woffsetlseek(fd, 0, SEEK_END))  -1 ) {
...
*****/
...
#if NEWIO
  retlio_write_buffer(fd, io_type, buf, grow_incr,
    SIGUSR1, &errmsg,0);
#else
  retwrite_buffer(fd, io_type, buf, grow_incr, 0, &errmsg);
#endif
  ...
  if( Mode & MODE_FIFO ){
   ...
   if( (tmplseek(fd,0,SEEK_CUR)) < 0 ){ /* get offset after the write */
 ...
 if ((cur_offsetlseek(fd,0,SEEK_CUR))  -1 ) {
  ...
  if ( new_offset < min_lseek )
      new_offsetmin_lseek;
                    ...
                    new_offsetrandom_range(min_lseek, new_offset, 1, NULL);
            }
            else {
                new_offsetrandom_range(min_lseek, max_lseek, 1, NULL);
            }

#ifdef CRAY
            if ((offsetlseek(fd, new_offset, SEEK_SET))  -1 ) {
...
int mode;       /* write mode */
{
 ...
 if ( Debug > 1 )  {
     ...
     */
#if NEWIO
    retlio_read_buffer(fd, io_type, Buffer, Grow_incr, SIGUSR1, &errmsg,0);
#else
    retread_buffer(fd, io_type, Buffer, Grow_incr, 0, &errmsg);
#endif
 ...
 for(ptrBuffer; ptr < (Buffer+Grow_incr-1); ptr++) {
     if ( *ptr ! '\0' ) {
         fprintf(stderr,
 ...
 retdatapidchk(STATIC_NUM, Buffer, Grow_incr, Woffset, &errmsg);
    else if ( Pattern  PATTERN_PID )
 retdatapidchk(Pid, Buffer, Grow_incr, Woffset, &errmsg);
    else if ( Pattern  PATTERN_ASCII )
        retdataasciichk(NULL, Buffer, Grow_incr, Woffset, &errmsg);
    else if ( Pattern  PATTERN_RANDOM )
 ; /* no check for random */
    else if ( Pattern  PATTERN_ALT )
        retdatabinchk('a', Buffer, Grow_incr, Woffset, &errmsg);
    else if ( Pattern  PATTERN_CHKER )
        retdatabinchk('c', Buffer, Grow_incr, Woffset, &errmsg);
    else if ( Pattern  PATTERN_CNTING )
  retdatabinchk('C', Buffer, Grow_incr, Woffset, &errmsg);
    else if ( Pattern  PATTERN_ZEROS )
    retdatabinchk('z', Buffer, Grow_incr, Woffset, &errmsg);
    else if ( Pattern  PATTERN_ONES )
    retdatabinchk('o', Buffer, Grow_incr, Woffset, &errmsg);
    else
 retdataasciichk(NULL, Buffer, Grow_incr, Woffset, &errmsg);
...
#if NEWIO
         retlio_read_buffer(fd, io_type, buf, rd_size,
      SIGUSR1, &errmsg,0);
#else
         retread_buffer(fd, io_type, buf, rd_size, 0, &errmsg);
#endif
...
***/
         ...
         if ( Pattern  PATTERN_OFFSET )
             retdatapidchk(STATIC_NUM, buf, rd_size, rd_cnt, &errmsg);
         else if ( Pattern  PATTERN_PID )
             retdatapidchk(Pid, buf, rd_size, rd_cnt, &errmsg);
         else if ( Pattern  PATTERN_ASCII )
             retdataasciichk(NULL, buf, rd_size, rd_cnt, &errmsg);
         else if ( Pattern  PATTERN_RANDOM )
      ;  /* no checks for random */
         else if ( Pattern  PATTERN_ALT )
             retdatabinchk('a', buf, rd_size, rd_cnt, &errmsg);
         else if ( Pattern  PATTERN_CHKER )
             retdatabinchk('c', buf, rd_size, rd_cnt, &errmsg);
         else if ( Pattern  PATTERN_CNTING )
             retdatabinchk('C', buf, rd_size, rd_cnt, &errmsg);
         else if ( Pattern  PATTERN_ZEROS )
             retdatabinchk('z', buf, rd_size, rd_cnt, &errmsg);
         else if ( Pattern  PATTERN_ONES )
             retdatabinchk('o', buf, rd_size, rd_cnt, &errmsg);
         else
             retdataasciichk(NULL, buf, rd_size, rd_cnt, &errmsg);
...
****/
#if NEWIO
     retlio_read_buffer(fd, io_type, buf, fsize, SIGUSR1, &errmsg,0);
#else
     retread_buffer(fd, io_type, buf, fsize, 0, &errmsg);
#endif
  ...
  ret_val1;
         ...
         if ( Pattern  PATTERN_OFFSET )
      retdatapidchk(STATIC_NUM, buf, fsize, 0, &errmsg);
         else if ( Pattern  PATTERN_PID )
      retdatapidchk(Pid, buf, fsize, 0, &errmsg);
  else if ( Pattern  PATTERN_ASCII )
      retdataasciichk(NULL, buf, fsize, 0, &errmsg);
  else if ( Pattern  PATTERN_RANDOM )
      ; /* no check for random */
  else if ( Pattern  PATTERN_ALT )
      retdatabinchk('a', buf, fsize, 0, &errmsg);
  else if ( Pattern  PATTERN_CHKER )
      retdatabinchk('c', buf, fsize, 0, &errmsg);
  else if ( Pattern  PATTERN_CNTING )
      retdatabinchk('C', buf, fsize, 0, &errmsg);
  else if ( Pattern  PATTERN_ZEROS )
      retdatabinchk('z', buf, fsize, 0, &errmsg);
  else if ( Pattern  PATTERN_ONES )
      retdatabinchk('o', buf, fsize, 0, &errmsg);
  else
      retdataasciichk(NULL, buf, fsize, 0, &errmsg);
