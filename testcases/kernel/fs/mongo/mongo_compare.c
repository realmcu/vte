...
main(int argc, char **argv)
{
    ...
    if( (f1fopen(argv[1],"r"))  NULL) {
 fprintf(stderr, "%s: can't open %s\n", argv[0], argv[1] );
    return 1;
    }

    if( (f2fopen(argv[2],"r"))  NULL) {
 fprintf(stderr, "%s: can't open %s\n", argv[0], argv[2] );
    return 1;
    }

    if( (f3fopen(out1,"wr"))  NULL) {
 fprintf(stderr, "%s: can't open %s\n", argv[0], out1 );
    return 1;
    }

    if( (f4fopen(out2,"wr"))  NULL) {
 fprintf(stderr, "%s: can't open %s\n", argv[0], out2 );
    return 1;
    }
  ...
  if ( p1strstr(line1, " :" )) {
    ...
    fprintf(f4, "<tr><td><tt> %s &nbsp </tt></td> <td><div alignright><tt> %6.2f / %6.2f  %.2f &nbsp </tt></div></td></tr>\n",
            name_str1, n1, n2, ratio);
