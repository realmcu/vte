...
void fill_buffer(void *buf, size_t len, void *pattern, size_t pattern_len, const unsigned int pattern_type)
{
 ...
 switch (pattern_type) { /* the pattern type should only be one of the following */
  ...
  case CLD_FLG_FPTYPE :
   ...
   off_tpat  0;
   for(j0;j<(sizeof(OFF_T)/pattern_len);j++)
    for(i0;i<pattern_len;++i)
#ifdef WINDOWS
  ...
  case CLD_FLG_RPTYPE :
    ...
    * at some point...
    */
   for(i0;i<BLK_SIZE/sizeof(OFF_T);i++)
    *(off_tbuf+i)  Rand64();

   for(iBLK_SIZE;i<len;i+BLK_SIZE)
