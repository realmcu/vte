...
#endif
{
 ...
 action_t target  { NONE, 0, 0 };
 unsigned int i;
 OFF_T ActualBytePos0, TargetBytePos0, mask1, delayMask1;
 long tcnt0;
 int exit_code0, rv0;
 ...
 while(env->bContinue) {
  ...
  ActualBytePosSeek(fd, TargetBytePos);
  ...
  if((target.oper  READER) && (args->flags & CLD_FLG_CMPR)) {
   ...
   pMsg(ERR, args, DMSTR, this_thread_id, target.lba, target.lba);
    ...
    for(i0;i<args->htrsiz*BLK_SIZE;i++) {
     if(*(buf2+i) ! *(buf1+i)) {
