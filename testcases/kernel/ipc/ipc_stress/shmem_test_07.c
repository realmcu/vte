...
intthread_hold[MAX_WRITER_NUMBER];
...
+---------------------------------------------------------------------*/
 ...
 j++;
 Sizesizeof (unsigned long) * num_readers;
...
void *writer (void *parm)
{
 ...
 unsigned long cksum_w  0; /* Shared memory regions checksum */
        ...
        for (ptrshmptr[num_w]; ptr < (shmptr[num_w]+buffer_size); ptr++) {
...
void *reader (void *parm)
{
 ...
 num_rnum_p % num_readers;
 num_wnum_p - num_r;
 num_wnum_w / num_readers;
 ptrshmptr[num_w];
 ulptr_rchecksum[num_w];
