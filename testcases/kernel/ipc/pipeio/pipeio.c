...
char *av[];
{
 ...
 int num_wrters1; /* number of writers */
 int num_writes1; /* number of writes per child */
 int loop0;  /* loop indefinitely */
 int exit_error  1; /* exit on error #, zero means no exit */
 int size327;  /* default size */
 int unpipe  0;  /* un-named pipe if non-zero */
 int verbose0;  /* verbose mode if set */
 int quiet0;  /* quiet mode if set */
 int num_rpt0;  /* ping number, how often to print message */
 ...
 if( (toutputgetenv("TOUTPUT")) ! NULL ) {
     if( strcmp( toutput, "NOPASS")  0 ) {
  quiet1;
     }
 }

        while ((cgetopt (ac, av, "T:BbCc:D:d:he:Ef:i:I:ln:p:qs:uvW:w:P:")) ! EOF) {
     switch (c) {
  case 'T':
   TCID  optarg;
   break;
  ...
  case 'f': /* format of buffer on error */
   ...
   case 'I':
    if((iotypelio_parse_io_arg1(optarg))  -1 ) {
            fprintf(stderr,
 ...
 if ( size > PIPE_BUF && num_wrters > 1 ) {
  ...
  num_writesMIN(((long long)num_writes*size+PIPE_BUF-1)/PIPE_BUF, INT_MAX/num_wrters);
...
printf("child after fork pid  %d\n", getpid());
 ...
 if (c > 0) { /***** if parent *****/
  ...
  for (inum_wrters*num_writes; i > 0 || loop; --i) {
...
prt_buf(long addr, char * buf, int length, int format)
{
 ...
 for(i0; i < num_words; ++i, a + NBPW, addr++) {
  word  (long *) a;
  if (!(i%width)) {
   if (i > 0 && format ! ASCII) {
       /*
