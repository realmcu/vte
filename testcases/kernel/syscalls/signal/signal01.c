...
char **argv;
{
    ...
    ***************************************************************/
    if ( (msgparse_opts(argc, argv, (option_t *) NULL, NULL)) ! (char *) NULL ) {
...
int tst_count;
{
  ...
  */
     ...
     while ( (rd_szread(fd1[0], (char *)&Ipc_info, sizeof(Ipc_info))) ! 0 ) {
         if ( rd_sz > 0 )
...
int test_case;
{
 ...
 Tretsignal(SIGKILL, SIG_IGN);
 TEST_ERRNOerrno;
    }
    else {
 exit_val  SIG_NOT_CAUGHT;
 strcpy(string, "signal(SIGKILL, catchsig)");
 Tretsignal(SIGKILL, catchsig);
 TEST_ERRNOerrno;
    }
    Ipc_info.timingstblock;
