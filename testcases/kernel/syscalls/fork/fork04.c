...
void child_environment()
{
  ...
  for (index0;index<NUMBER_OF_ENVIRON;index++) {
      memset(msg, 0, MAX_LINE_LENGTH);

      if ( (vargetenv(environ_list[index]))  NULL )
...
void parent_environment()
{
  ...
  for (index0;index<NUMBER_OF_ENVIRON;index++)
    {
      if ((retread(fildes,tmp_line,MAX_LINE_LENGTH))  0) {
