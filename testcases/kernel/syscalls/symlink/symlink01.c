...
struct tcses
{
  ...
  "Changes CWD Location Indirectly from a symlink" },
   /* Spring, 2009.2.6, See Line 431 comments, only 2 cases are valid, the third is BROK */
...
main(int argc, char *argv[])
{
    ...
    */
   if ((tcs_ptrget_tcs_info(Selectedtests))  NULL) {
      TST_TOTAL1;
...
char *ptr;
{
 ...
 TST_TOTAL0;
 for (ctr1; ctr < sizeof(all_tcses)/sizeof(struct tcses); ctr++)
...
char *path1, *path2, *path3;
{
   ...
   if ((retsee_if_a_symlink(path2))  -1) {
      TEST_RESULTTBROK;
      sprintf(test_msg,
...
char *path1, *path2, *path3;
{
 ...
 path1, path2, path3);
   ...
   for (ctr0; ctr < whole_chunks; ctr++) {
       strcat(full_path, path1);
   }
   size strlen(full_path);
   to_go  PATH_MAX - size;
   strcat(full_path, "/");
   for (ctr0; ctr < to_go; ctr++)
       strcat(full_path, "Z");
...
char *path1, *path2, *path3;
{
    ...
    if ((retsee_if_a_symlink(path1)) < 0) {
       TEST_RESULTTFAIL;
       sprintf(test_msg,
...
struct tcses *tcs;
{
    ...
    */
   for (ctr0, tc_ptrtcs->tc_ptr; ctr < TST_TOTAL; ctr++, tc_ptr++) {
...
struct all_test_cases *tc_ptr;
{
 ...
 retreadlink(full_path, scratch, strlen(full_path));
...
struct all_test_cases *tc_ptr;
{
     ...
     retreadlink(tc_ptr->fn_arg[1], scratch, strlen(tc_ptr->fn_arg[0]));
...
struct all_test_cases *tc_ptr;
{
 ...
 errnoTEST_ERRNO;
     ...
     if ((retwrite(fd, BIG_STRING, strlen(BIG_STRING)))  -1) {
        tst_resm(TFAIL,
     ...
     else if ((retread(fd, scratch, strlen(BIG_STRING)))  -1) {
        tst_resm(TFAIL,
     ...
     BIG_STRING, scratch);
        ...
        */
        else if ((fdopen(tc_ptr->fn_arg[0], O_RDONLY))  -1) {
...
void help()
{
    ...
    for (ind0; ind<sizeof(all_tcses)/sizeof(struct tcses ); ind++) {
