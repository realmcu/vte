 ...
 * param2                  *
 * param3                  *
 * param4    if sched_tc6:  fork flag:  0false, 1true             *
 *-----------------------------------------------------------------------*/
 ...
 *param1,   /* user flag:  0user, 1real time */
 *param2,   /* priority to run the throughput test at */
 *param3;   /* if sched_tc6:  fork flag, 0false, 1true */
...
void parse_args (int argc, char **argv)
{
 ...
 while ((opt  getopt(argc, argv, "vs:p:t:l:d")) ! EOF)
 {
  ...
  case 'd': /* Enable debugging messages */
   debug++;
   break;
  case 'v': /* Enable verbose modedebug mode */
   debug++;
   break;
  default:
   errflag++;
