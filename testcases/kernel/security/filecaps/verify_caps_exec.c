...
int caps_actually_set_test(void)
{
 ...
 for (whichcap0; whichcap < NUM_CAPS; whichcap++) {
  /*
   * fPwhichcap, fEfI0
   * pP'whichcap, pI'pE'0
   */
  ...
  if (ret) {
   tst_resm(TINFO, "Failed CAP_PERMITTED%d CAP_EFFECTIVE0\n",
     whichcap);
...
/* SERGE here */
  ...
  ret  fork_drop_and_exec(DROP_PERMS, fcap);
  if (ret) {
   tst_resm(TINFO, "Failed CAP_PERMITTED%d CAP_EFFECTIVE1\n",
    whichcap);
   if (!finalret) finalret  ret;
  }
 }
 ...
 for (whichcap0; whichcap < NUM_CAPS; whichcap++) {
  ...
  /*
   * fIfEwhichcap, fP0  (so fcap is same as before)
   * pI0  (achieved using DROP_PERMS)
   * pI'pP'pE'0
   */
  cap_clear(pcap);
