 ...
 *
 *  1 Jan 2009  0.3  Test cases added for indexS32_MAX and S32_MAX+1;
...
static void do_enumerate_formats(enum v4l2_buf_type type) {
 ...
 do {
  ...
  if (ret  0) {
   ...
   dprintf("\tformat  {.index%u, .type0x%X, .flags0x%X, "
    ".description\"%s\", .pixelformat0x%X, "
