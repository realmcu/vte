...
void test_VIDIOC_QUERYCTRL_flag_NEXT_CTRL() {
 ...
 dprintf("\tasking for id%iV4L2_CID_BASE+%i | V4L2_CTRL_FLAG_NEXT_CTRL\n", i, i-V4L2_CID_BASE);
 ret  ioctl(get_video_fd(), VIDIOC_QUERYCTRL, &queryctrl);
 dprintf("\tret%i\n", ret);
 ...
 if (ret  0) {
  ...
  for (i0; i<sizeof(count_controls1)/sizeof(*count_controls1); i++) {
      dprintf("%i ", count_controls1[i]);
  }
  dprintf1("}\n");

  dprintf1("count_controls2  { ");
  for (i0; i<sizeof(count_controls2)/sizeof(*count_controls2); i++) {
