...
int main(int argc, char* argv[]) {
  char* mem;
  float percent;
  unsigned int maxpercent0, dowrite0, verbose0, j, c;
  unsigned long bytecount, alloc_bytes;
  unsigned long long original_maxbytes,maxbytes0;
  ...
  while((cgetopt(argc, argv, "c:b:p:wvh")) ! EOF) {
    switch((char)c) {
      case 'c':
 ...
 if (maxpercent > 99){
  ...
  if(maxpercent) {
    ...
    total_ramsstats.totalram;
    total_ramtotal_ram+sstats.totalswap;

    total_freesstats.freeram;
    total_freetotal_free+sstats.freeswap;
  ...
  {
    i++;
    maxbytesmaxbytes-(500*1024*1024);
    pidfork();
    if (pid ! 0)
      pid_cntr++;
      pid_list[i]pid;
  ...
  {
    i++;
    maxbytesmaxbytes-(1024*1024*1024);
    pidfork();
    if (pid ! 0)
      pid_cntr++;
      pid_list[i]pid;
  ...
  {
    i++;
    maxbytesmaxbytes-(unsigned long long)3*1024*1024*1024;
    pidfork();
    if (pid ! 0)
      pid_cntr++;
      pid_list[i]pid;
