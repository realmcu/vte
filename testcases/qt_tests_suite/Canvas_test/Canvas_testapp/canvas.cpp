    ...
    canvas(c)
{
    ...
    file->insertItem( "&Exit - Test Pass",  this, SLOT(exitPass()), CTRL+Key_Q );
    file->insertItem( "&Qiut - Test Fail", this, SLOT(exitFail()), CTRL+Key_C );
...
void Main::addHexagon()
{
    ...
    i->setPoints(pa);
    i->setBrush( QColor(rand()%32*8,rand()%32*8,rand()%32*8) );
    i->move(rand()%(canvas.width()-size),rand()%(canvas.height()-size));
    i->setZ(z++);
    i->show();
}
...
void Main::addPolygon()
{
    ...
    i->setPoints(pa);
    i->setBrush( QColor(rand()%32*8,rand()%32*8,rand()%32*8) );
    i->move(rand()%(canvas.width()-16),rand()%(canvas.height()-16));
    i->setZ(z++);
    i->show();
}
...
{
	int h = rand()%(canvas.height()/5)+18;
	int w = rand()%(canvas.width()/5)+18;
    int x = rand()%(canvas.width()-w);
	int y = rand()%(canvas.height()-h);
    
	QCanvasPolygonalItem *i = new QCanvasRectangle( x,y,w,h,&canvas);
    int c = rand()%256;
    i->setBrush( QColor(c,c,c) );
    i->setPen( QPen(QColor(rand()%32*8,rand()%32*8,rand()%32*8), 6) );
    i->setZ(z++);
    i->show();
}
