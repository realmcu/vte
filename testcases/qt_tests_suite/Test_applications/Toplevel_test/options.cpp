...
**      by: The User Interface Compiler ($Id: options.cpp,v 1.1.1.1 2008/04/14 09:01:41 b06080 Exp $)
**
    ...
    : QDialog( parent, name, modal, fl )
{
    ...
    gbProperties->layout()->setMargin( 8 );
...
*/
    ...
    pbClose_E = new QPushButton( this, "pbClose" );
    layout1->addWidget( pbClose_E );
//    OptionsDialogLayout->addLayout( layout1 );
    pbClose_E->setText(  "&EXIT - fail"  );


//1    setFixedSize(240,320);
//1    resize( QSize(240,320).expandedTo(minimumSizeHint()) );
    clearWState( WState_Polished );

    // signals and slots connections
//    connect( pbClose, SIGNAL( clicked() ), this, SLOT( close() ) );
    connect( pbClose, SIGNAL( clicked() ), this, SLOT( exitPass() ) );
    connect( pbClose_E, SIGNAL( clicked() ), this, SLOT( exitFail() ) );
...
"Note that this attribute is not supported on many windowing systems and will have no effect." ) );
    ...
    pbClose->setText( tr( "QUIT - pass" ) );
    pbClose->setAccel( QKeySequence( QString::null ) );
    QWhatsThis::add( pbClose, tr( "Closes this dialog and exiting with test pass the application." ) );
//    pbClose_E->setText( tr( "&EXIT - fail" ) );
/*    pbClose_E->setAccel( QKeySequence( QString::null ) );
    QWhatsThis::add( pbClose_E, tr( "Closes this dialog and exiting with test fail the application." ) );
  */
