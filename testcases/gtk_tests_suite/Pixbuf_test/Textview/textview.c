...
char *demo_find_file (char *filename,GtkWindow *window)
{
    GError *error;
    gchar *filename_1;
    GDir *dir;
    ...
    pathget_current_dir_name();
    dirg_dir_open (path,0,&error);
     while(filename_1)
      {
...
static void
insert_text (GtkTextBuffer *buffer,GtkWindow *window)
{
  GtkTextIter iter;
   ...
   */
  pixbuf  NULL;
  filename demo_find_file ("gtk-logo-rgb.gif",window);
  if (filename)
    {
...
static void
attach_widgets (GtkTextView *text_view,GtkWindow *window)
{
  GtkTextIter iter;
  ...
  while (find_anchor (&iter))
    {
      ...
      if (i  0)
        ...
        {
   gchar *filename demo_find_file("floppybuddy.gif",window);
...
/////////
      ...
      create_tags (buffer);
      insert_text (buffer,GTK_WINDOW(window));

      attach_widgets (GTK_TEXT_VIEW (view1),GTK_WINDOW(window));
      attach_widgets (GTK_TEXT_VIEW (view2),GTK_WINDOW(window));
