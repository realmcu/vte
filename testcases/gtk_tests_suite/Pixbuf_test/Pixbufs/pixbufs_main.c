...
/* Global Variables */
char *TCID      "pixbufs_testapp_X"; /* test program identifier.          */
...
void cleanup()
{
  ...
  VT_rv  VT_pixbufs_cleanup();
  if (VT_rv ! TPASS)
  {
   tst_resm(TWARN, "VT_pixbufs_cleanup() Failed : error code  %d", VT_rv);
  }
  /* VTE */
...
void setup()
{
 int VT_rv  TFAIL;
  ...
  VT_rv  VT_pixbufs_setup();
  if (VT_rv ! TPASS)
  {
   tst_brkm(TBROK , cleanup, "VT_pixbufs_setup() Failed : error code  %d", VT_rv);
  }
  /* VTE */
...
int main(int argc, char **argv)
{
 int VT_rv  TFAIL;
    ...
    VT_rv  VT_pixbufs_test_X( argc, argv); /*with the parameters needed come from parse_opt())*/
