      ...
      const char *prompt)
{
  ...
  if (gdk_pointer_grab (popup->window, FALSE,
   ...
   G_CALLBACK (button_release_event_cb), &clicked);
       ...
       * We pass in may_blockTRUE since we want to wait if there
 ...
 */
static void
close_display_cb (GtkWidget *button,ChangeDisplayInfo *info)
{
 ...
 */
  ...
  selection  gtk_tree_view_get_selection (GTK_TREE_VIEW (tree_view));
  g_signal_connect (selection, "changed",  G_CALLBACK (display_changed_cb), info);
...

{
   ChangeDisplayInfo *info;
    gtk_init (&argc,&argv);
      ...
      info->window  gtk_dialog_new ();
      gtk_window_set_default_size (GTK_WINDOW (info->window), 240, 320);
      gtk_signal_connect (GTK_OBJECT (info->window), "destroy",
                             GTK_SIGNAL_FUNC (gtk_main_quit), &(info->window));

     gtk_window_set_policy(GTK_WINDOW (info->window),TRUE,TRUE,TRUE);
     ...
     TRUE, TRUE, 0);
      ...
      button  left_align_button_new ("_Change");
      g_signal_connect (button, "clicked",  G_CALLBACK (response_cb), info);
      gtk_box_pack_start (GTK_BOX (hbox), button, TRUE,FALSE , 0);

      button  left_align_button_new ("_Quit-pass");
      g_signal_connect (button, "clicked",  G_CALLBACK (destroy_Quit), info);
      gtk_box_pack_start (GTK_BOX (hbox), button, TRUE, FALSE, 0);

      button  left_align_button_new ("_Exit-fail");
      g_signal_connect (button, "clicked",  G_CALLBACK (destroy_Exit), info);
      gtk_box_pack_start (GTK_BOX (hbox), button,TRUE, FALSE, 0);
