...
/* Global Variables */
char *TCID     = "rangewidgets_testapp_X"; /* test program identifier.          */
...
void cleanup()
{
    /* VTE : Actions needed to get a stable target environment */
		...
		VT_rv = VT_rangewidgets_cleanup();
		if (VT_rv != TPASS)
		{
			tst_resm(TWARN, "VT_rangewidgets_cleanup() Failed : error code = %d", VT_rv);
		}
		/* VTE */
...
void setup()
{
   ...
   /* Capture signals. */
    ...
    /* VTE : Actions needed to prepare the test running */
		VT_rv = VT_rangewidgets_setup();
		if (VT_rv != TPASS)
		{
			tst_brkm(TBROK , cleanup, "VT_rangewidgets_setup() Failed : error code = %d", VT_rv);
		}
		/* VTE */
...
int main(int argc, char **argv)
{
    ...
    VT_rv = VT_rangewidgets_test_X( argc, argv); /*with the parameters needed come from parse_opt())*/
