...
/*================================================================================================*/
/**
        @file   nand_mtd_main.c

        @brief   NAND MTD test main function.
====================================================================================================
Revision History:
...
/* Global Variables */
char   *TCID = "nand_mtd";      /* test program identifier.  */
...
void cleanup(void)
{
        ...
        VT_rv = VT_nand_mtd_test_cleanup();
        if (VT_rv != TPASS)
        {
                tst_resm(TWARN, "VT_nand_mtd_cleanup() Failed : error code = %d", VT_rv);
        }
...
void setup(void)
{
        ...
        VT_rv = VT_nand_mtd_test_setup();
        if (VT_rv != TPASS)
        {
                tst_brkm(TBROK, cleanup, "VT_nand_mtd_setup() Failed : error code = %d", VT_rv);
        }
...
int main(int argc, char **argv)
{
        ...
        {
                strcpy(device_name, NAND_MTD_DEVICE);
        }
        ...
        else
        {
                ...
                else
                {
				     ...
				     if(strcmp(t_copt,TEST_CASE_GETBADBLOCK)==0)
                        	    	{
                        	    		VT_rv=VT_nand_mtd_test_regionInfo();
						VT_rv=VT_nand_mtd_test_badblk();
						if(VT_rv==TPASS)
						{
				    ...
				    else
				    	{
				    		...
				    		{
							VT_rv=VT_nand_mtd_test_perform();
						...
						else
						{
							...
							{
								VT_rv=VT_nand_mtd_test_thrdrwe();
								if(VT_rv==TPASS)
								{
									tst_resm(TPASS,"test case %s worked as expected",TCID);
								}
							...
							else
							{
								...
								{
									VT_rv=VT_nand_mtd_test_thrdrwonepage();
									if(VT_rv==TPASS)
										{
										tst_resm(TPASS,"test case %s worked as expected",TCID);
									}
