...
V.BECKER/rc023c              21/10/2004     TLSbo43924  Correct test VT_nor_mtd_test_wrner
...
int VT_nor_mtd_test_info(void)
{
        ...
        {
                tst_resm(TFAIL, "VT_nor_mtd_test() Failed ioctl MEMGETREGIONCOUNT");
                return TFAIL;
        }
...
int VT_nor_mtd_test_regionInfo(void)
{
        int     number_of_regions = 0;
...
int VT_nor_mtd_test_setup(void)
{
        ...
        if (file_desc < 0)
        {
                tst_resm(TFAIL, "VT_nor_mtd_test_setup() Failed open device");
                return TFAIL;
        }
        if (v_num)
        {
        ...
        {
                tst_resm(TFAIL, "VT_nor_mtd_test() Failed allocate memory");
                return TFAIL;
        }
        ...
        {
                tst_resm(TFAIL, "VT_nor_mtd_test() Failed ioctl MEMGETINFO");
                return TFAIL;
        }
...
int erase_flash_block(long offset, long length)
{
        ...
        for (i = offset; i < (offset + length); i += mxc_info_mtd.erasesize)
        {
                ...
                {
                        tst_resm(TFAIL, "%d: MTD get bad block failed:%s ",file_desc,strerror(errno));
                        exit(1);
                }
                else
                */
