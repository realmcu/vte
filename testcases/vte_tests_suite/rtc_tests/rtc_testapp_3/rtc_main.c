...
void cleanup(void)
{
        ...
        VT_rv = VT_rtc_test3_cleanup();
        if (VT_rv != TPASS)
        {
                tst_resm(TWARN, "VT_cleanup() Failed : error code = 0", VT_rv);
        }
...
int main(int argc, char **argv)
{
        ...
        tst_resm(TINFO, "Testing if  test case is OK", TCID);

        /* VTE : print results and exit test scenario */
        VT_rv = VT_rtc_test3(); /*with the parameters needed come from parse_opt())*/

        if(VT_rv == TPASS)
        {
                tst_resm(TPASS, " test case worked as expected", TCID);
        }
        else
        {
