...
*/
	int parse_options(void) {
		...
		case PRP_ENC_TO_F:
			...
			break;
		default:
			tst_resm(TWARN, "Case number is erroneous : %d",
		...
		if (gV4LTestConfig.mOutputFormat) {
			...
			if (i == PIX_FMT_NUM) {
				tst_resm(TWARN,
					 "The pixel format %s is not supported by the driver",
					 gV4LTestConfig.mPixFormat);
				return TFAIL;
			}
		}
...
*/
	int open_device(void) {
		...
		if (stat(gV4LTestConfig.mV4LDevice, &st) < 0) {
			tst_resm(TBROK, "Cannot identify '%s': %d, %s",
				 gV4LTestConfig.mV4LDevice, errno,
				 strerror(errno));
			return TFAIL;
		}
		if (!S_ISCHR(st.st_mode)) {
			tst_resm(TBROK, "%s is not a char device",
				 gV4LTestConfig.mV4LDevice);
			return TFAIL;
		}
		if (gV4LTestConfig.mIsBlock == 0) {
			if ((gFdV4L =
			     open(gV4LTestConfig.mV4LDevice,
				  O_RDWR | O_NONBLOCK, 0)) < 0) {
				tst_resm(TBROK, "Unable to open %s",
					 gV4LTestConfig.mV4LDevice);
				return TFAIL;
			}
		} else {
			if ((gFdV4L =
			     open(gV4LTestConfig.mV4LDevice, O_RDWR, 0)) < 0) {
				tst_resm(TBROK, "Unable to open %s",
					 gV4LTestConfig.mV4LDevice);
				return TFAIL;
			}
...
*/
	int close_device(void) {
		...
		if ((gFdV4L > 0) && (close(gFdV4L) < 0)) {
			tst_resm(TBROK, "Unable to close %s",
				 gV4LTestConfig.mV4LDevice);
			retValue = TFAIL;
		}
		if (gpFB_buf != NULL)
...
*/
	int open_out_device(void) {
		...
		gVideoBufferSize =
		    gScreenInfo.xres * gScreenInfo.yres *
		    gScreenInfo.bits_per_pixel / 8;
		if ((gpFB =
		     mmap(0, gVideoBufferSize, PROT_READ | PROT_WRITE,
			  MAP_SHARED, gFdFB, 0)) < 0) {
			tst_resm(TBROK,
...
*/
		...
		if (ioctl(gFdV4L, VIDIOC_G_FBUF, &fbuffer) < 0) {
			tst_resm(TWARN,
				 "Error VIDIOC_G_FBUF : Overlay is not supported by device %s",
				 gV4LTestConfig.mV4LDevice);
			return TFAIL;
		}
		...
		if (gV4LTestConfig.mOverlayType == V4L2_FBUF_FLAG_PRIMARY) {
			if ((gFdFB =
			     open(gV4LTestConfig.mOutputDevice, O_RDWR)) < 0) {
				tst_resm(TBROK, "Unable to open frame buffer");
				return TFAIL;
			}
			if (ioctl(gFdFB, FBIOGET_VSCREENINFO, &gScreenInfo)) {
				tst_resm(TBROK,
					 "Unable to read FB var information");
				return TFAIL;
			}
			if (ioctl(gFdFB, FBIOGET_FSCREENINFO, &fScreenInfo)) {
				tst_resm(TBROK,
					 "Unable to read FB fix information");
				return TFAIL;
			}
			fbuffer.fmt.width = gScreenInfo.xres;
			fbuffer.fmt.height = gScreenInfo.yres;
			fbuffer.fmt.bytesperline =
			    gScreenInfo.bits_per_pixel / 8 * fbuffer.fmt.width;
		...
		if (ioctl(gFdV4L, VIDIOC_G_PARM, &streamParm) < 0) {
			tst_resm(TWARN,
				 "ERROR init_overlay() : get frame rate failed");
			return TFAIL;
		}
		...
		if (gV4LTestConfig.mVerbose) {
			tst_resm(TINFO,
				 "Original image size: left = %d, top = %d, width = %d, height = %d",
				 gOrigFormatRect.left, gOrigFormatRect.top,
		...
		if (gV4LTestConfig.mCrop) {
			...
			tst_resm(TINFO,
				 ...
				 gFormat.fmt.win.w.left, gFormat.fmt.win.w.top,
				 gFormat.fmt.win.w.width,
				 gFormat.fmt.win.w.height);
		} else {
			...
			tst_resm(TINFO,
				 "Default image: left = %d, top = %d, width = %d, height = %d",
				 gFormat.fmt.win.w.left, gFormat.fmt.win.w.top,
				 gFormat.fmt.win.w.width,
				 gFormat.fmt.win.w.height);
		}
		...
		if (ioctl(gFdV4L, VIDIOC_S_FBUF, &fbuffer) < 0) {
			...
			case EACCES:
				tst_resm(TWARN,
					 "Error VIDIOC_S_FBUF can only be called by a privileged user");
				break;
			case EINVAL:
				tst_resm(TWARN,
		...
		tst_resm(TINFO,
			 "VIDIOC_S_FBUF executed - overlay type %s is set",
			 gV4LTestConfig.mOverlayType ==
			 V4L2_FBUF_FLAG_PRIMARY ? "PRIMARY" : "OVERLAY");
		return TPASS;
	}
...
*/
	int init_capture(void) {
		...
		if (gV4LTestConfig.mVerbose) {
			tst_resm(TINFO,
				 "Original image size: width = %d, height = %d",
				 gOrigFormatRect.width, gOrigFormatRect.height);
		...
		if (gV4LTestConfig.mVerbose) {
			tst_resm(TINFO, "\tCapture : Format image width = %d",
				 gFormat.fmt.pix.width);
...
*/
	int init_mmap(void) {
		...
		if (ioctl(gFdV4L, VIDIOC_REQBUFS, &reqBuffers) < 0) {
			tst_resm(TWARN,
				 "%s does not support memory mapping : %s",
				 gV4LTestConfig.mV4LDevice, strerror(errno));
			return TFAIL;
		}
		if (reqBuffers.count < 2) {
			tst_resm(TWARN, "Insufficient buffer memory on %s",
				 gV4LTestConfig.mV4LDevice);
			return TFAIL;
		}
		...
		for (gBuffNumber = 0; gBuffNumber < reqBuffers.count;
		     gBuffNumber++) {
			CLEAR(buffer);
			...
			gpBuffers[gBuffNumber].mpStart =
			    mmap(NULL, buffer.length, PROT_READ | PROT_WRITE,
				 MAP_SHARED, gFdV4L, buffer.m.offset);
...
#define MX25_TEMP_SLT 0
				...
				if (ioctl(fd_fb, FBIOGET_FSCREENINFO, &fix) < 0) {
					printf
					    ("Unable to get frame buffer parameter\n");
					return TFAIL;
				}
#endif
				...
				if (ioctl(gFdV4L, VIDIOC_OVERLAY, &start) < 0) {
					tst_resm(TWARN,
						 "Error start_capturing() for VIDIOC_OVERLAY : %s",
						 strerror(errno));
					return TFAIL;
				}
			...
			break;
		default:
			tst_resm(TWARN, "The buffer type %d is not supported",
				 gUsecase);
			return TFAIL;
		}
		gStartStream = 1;
	...
	int stop_capturing(void) {
		...
		case V4L2_BUF_TYPE_VIDEO_CAPTURE:
			...
			if (ioctl(gFdV4L, VIDIOC_STREAMOFF, &typeBuffer) < 0) {
				tst_resm(TWARN,
					 "Error stop_capturing() for VIDIOC_STREAMOFF : %s",
					 strerror(errno));
				return TFAIL;
			}
			break;
...
*/
	int init_device(void) {
		...
		if (ioctl(gFdV4L, VIDIOC_QUERYCAP, &cap) < 0) {
			tst_resm(TWARN, "%s is not a v4l2 device",
				 gV4LTestConfig.mV4LDevice);
			return TFAIL;
		}
#if 1
		...
		if (gUsecase == V4L2_BUF_TYPE_VIDEO_CAPTURE) {
			if (!(cap.capabilities & V4L2_CAP_VIDEO_CAPTURE)) {
				tst_resm(TWARN, "%s does not support capturing",
					 gV4LTestConfig.mV4LDevice);
				return TFAIL;
			}
...
*/
	int config_device_for_rotation(int aRotNum) {
		...
		if (gV4LTestConfig.mRotation) {
			...
			case 2:
				tst_resm(TINFO,
					 "Rotation type: horizontal flip");
				break;
			case 3:
				tst_resm(TINFO,
					 "Rotation type: on 180 degrees");
				break;
			case 4:
				tst_resm(TINFO,
					 "Rotation type: on 90 degrees clock-wise");
				break;
			case 5:
				tst_resm(TINFO,
					 "Rotation type: on 90 degrees clock-wise + "
			...
			case 7:
				tst_resm(TINFO,
					 "Rotation type: on 90 degrees counter-clock-wise");
				break;
			default:
			...
			if (ioctl(gFdV4L, VIDIOC_S_CTRL, &control) < 0) {
				tst_resm(TBROK,
					 "Error: Rotation configuration failed");
				return TFAIL;
			}
			/* Check the result  */
			if (ioctl(gFdV4L, VIDIOC_G_CTRL, &control) < 0) {
				if (control.value != aRotNum) {
					tst_resm(TBROK,
						 "Error: Rotation option %d is not supported",
						 aRotNum);
					return TFAIL;
				}
			}
	...
	int reset_device(void) {
		...
		if (gV4LTestConfig.mCrop) {
			...
			if (ioctl(gFdV4L, VIDIOC_S_CROP, &crop) < 0) {
				tst_resm(TBROK,
					 "Error: VIDIOC_S_CROP ioctl failed");
				return TFAIL;
			}
			if (gV4LTestConfig.mVerbose) {
				tst_resm(TINFO,
					 "Reinit to default cropping settings: left = %d, top = %d, width = %d, height = %d",
		...
		if (gV4LTestConfig.mVerbose) {
			if (gUsecase == V4L2_BUF_TYPE_VIDEO_OVERLAY)
				tst_resm(TINFO,
					 "Reinit to default image size: left = %d, top = %d, width = %d, height = %d",
					 gFormat.fmt.win.w.left,
					 gFormat.fmt.win.w.top,
					 gFormat.fmt.win.w.width,
					 gFormat.fmt.win.w.height);
			else {
				tst_resm(TINFO,
					 "Reinit to default image size:  width = %d, height = %d",
					 gFormat.fmt.pix.width,
					 gFormat.fmt.pix.height);
				if (gFormat.fmt.pix.pixelformat ==
				    gOrigPixFormat)
					tst_resm(TINFO,
						 "Reinit to default pixel format: %s",
						 gOrigPixFmtName);
				else
					tst_resm(TINFO,
						 "Not reinit to default pixel format: %s",
						 gPixFmtName);
			}
		}
		return TPASS;
	...
	int process_image(const unsigned char *aStart, int aLength) {
		...
		case PRP_ENC_TO_F:
			if (gSnapshot) {
				...
				sprintf(outFileName, "%s%s%s",
					gV4LTestConfig.mOutputFile, "_",
					gPixFmtName);
		...
		case PRP_VF:
			if (gV4LTestConfig.mVerbose) {
				tst_resm(TINFO,
					 "process_image() : displaying overlay...");
			}
			break;
		}
		return TPASS;
...
*/
	int read_frame(void) {
		...
		case V4L2_BUF_TYPE_VIDEO_CAPTURE:
			...
			if (buffer.index == 0
			    && (gV4LTestConfig.mCaseNum == PRP_ENC_ON_D
				|| (gV4LTestConfig.mCaseNum == PRP_ENC_TO_F
				    && gSnapshot))) {
				memcpy(gpFB_buf, gpBuffers[0].mpStart,
...
*/
	int write_file_header(void) {
		char outFileName[MAX_STR_LEN];
		sprintf(outFileName, "%s%s%s", gV4LTestConfig.mOutputFile, "_",
			gPixFmtName);
		if (gV4LTestConfig.mVerbose)
		...
		if (!(pDumpFile = fopen(outFileName, "w+b"))) {
			...
			fwrite(gPixFmtName, sizeof(unsigned char),
			       strlen(gPixFmtName), pDumpFile);
			putc('\0', pDumpFile);
			write_zero_to_file(pDumpFile,
					   0x000F - strlen(gPixFmtName));
			fwrite(width, sizeof(unsigned char), strlen(width),
			       pDumpFile);
			putc('\0', pDumpFile);
			write_zero_to_file(pDumpFile,
					   (0x001F - 0x0010) - strlen(width));
			fwrite(height, sizeof(unsigned char), strlen(height),
			       pDumpFile);
			putc('\0', pDumpFile);
			write_zero_to_file(pDumpFile,
					   (0x002F - 0x0020) - strlen(height));
...
*/
	int detect_fb_fmt(void) {
		...
		case 8:
			if ((gScreenInfo.red.offset == 5)
			    && (gScreenInfo.red.length == 3)
			    && (gScreenInfo.green.offset == 2)
			    && (gScreenInfo.green.length == 3)
			    && (gScreenInfo.blue.offset == 0)
			    && (gScreenInfo.blue.length == 2)) {
				gFBPixFormat = V4L2_PIX_FMT_RGB332;
				strcpy(gFBPixFmtName, "RGB332");
			}
			break;
		case 16:
			if ((gScreenInfo.red.offset == 11)
			    && (gScreenInfo.red.length == 5)
			    && (gScreenInfo.green.offset == 5)
			    && (gScreenInfo.green.length == 6)
			    && (gScreenInfo.blue.offset == 0)
			    && (gScreenInfo.blue.length == 5)) {
			...
			if ((gScreenInfo.red.offset == 11)
			    && (gScreenInfo.red.length == 5)
			    && (gScreenInfo.green.offset == 6)
			    && (gScreenInfo.green.length == 5)
			    && (gScreenInfo.blue.offset == 1)
			    && (gScreenInfo.blue.length == 5)) {
				gFBPixFormat = V4L2_PIX_FMT_RGB555X;
				strcpy(gFBPixFmtName, "RGB555X");
				break;
			}
			if ((gScreenInfo.red.offset == 0)
			    && (gScreenInfo.red.length == 5)
			    && (gScreenInfo.green.offset == 5)
			    && (gScreenInfo.green.length == 6)
			    && (gScreenInfo.blue.offset == 11)
			    && (gScreenInfo.blue.length == 5)) {
				gFBPixFormat = V4L2_PIX_FMT_RGB565;
			...
			if (gV4LTestConfig.mVerbose) {
				...
				printf("\t Red\t%d\t%d\n",
				       gScreenInfo.red.offset,
				       gScreenInfo.red.length);
				printf("\t Green\t%d\t%d\n",
				       gScreenInfo.green.offset,
				       gScreenInfo.green.length);
				printf("\t Blue\t%d\t%d\n",
				       gScreenInfo.blue.offset,
				       gScreenInfo.blue.length);
			}
			return TFAIL;
		case 24:
			if ((gScreenInfo.red.offset == 16)
			    && (gScreenInfo.red.length == 8)
			    && (gScreenInfo.green.offset == 8)
			    && (gScreenInfo.green.length == 8)
			    && (gScreenInfo.blue.offset == 0)
			    && (gScreenInfo.blue.length == 8)) {
		...
		if (gV4LTestConfig.mVerbose)
			tst_resm(TINFO,
				 "detect_fb_fmt() has detect framebuffer pix format : %s",
				 gFBPixFmtName);
		return TPASS;
	}
...
*/
	void display_to_fb(unsigned char *aStart, int aLength) {
		int dstWidth =
		    gScreenInfo.xres * gScreenInfo.bits_per_pixel / 8;
		int srcWidth =
		    gFormat.fmt.pix.width * gScreenInfo.bits_per_pixel / 8;
...
*/
	int do_cropping(void) {
		...
		if (gV4LTestConfig.mVerbose) {
			tst_resm(TINFO,
				 "Cropping bounds: left = %d, top = %d, width = %d, height = %d",
		...
		if (gV4LTestConfig.mVerbose) {
			tst_resm(TINFO,
				 "Default cropping restangle: left = %d, top = %d, width = %d, height = %d",
				 crop.c.left, crop.c.top, crop.c.width,
				 crop.c.height);
		}
...
*/
	int setup_device(void) {
		...
		if (init_device() != TPASS) {
			tst_resm(TWARN, "%s init failed",
				 gV4LTestConfig.mV4LDevice);
			return TFAIL;
		}
		if (start_capturing() != TPASS) {
			tst_resm(TWARN, "%s capturing start failed",
				 gV4LTestConfig.mV4LDevice);
			return TFAIL;
		}
		return TPASS;
...
*/
	int cleanup_device(void) {
		...
		if (stop_capturing() != TPASS) {
			tst_resm(TBROK, "%s capturing stop failed",
				 gV4LTestConfig.mV4LDevice);
			return TFAIL;
		}
		if (reset_device() != TPASS) {
			tst_resm(TBROK, "%s reset failed",
				 gV4LTestConfig.mV4LDevice);
			return TFAIL;
		}
		if (uninit_device() != TPASS) {
			tst_resm(TBROK, "%s uninit failed",
				 gV4LTestConfig.mV4LDevice);
			return TFAIL;
		}
		return TPASS;
...
*/
	int register_conf(int div_ratio) {
		...
		if (div_ratio < 0 || div_ratio > 255)
			div_ratio = 4;
		if ((gmap_fd = open("/dev/mem", O_RDWR)) < 0) {
			tst_resm(TFAIL, "Error open /dev/mem : %s",
				 strerror(errno));
			return TFAIL;
		}
		pCSIReg = mmap(NULL, 256, PROT_READ | PROT_WRITE, MAP_SHARED, gmap_fd, 0x53FC0000	//0x53FC0000 - the initial address CSI registers
		    );
		if (MAP_FAILED == pCSIReg) {
			tst_resm(TFAIL, "%s : Can't mmap /dev/mem %s",
				 strerror(errno));
			return TFAIL;
		}
		...
		*pCSI_sens_conf_reg =
		    (read_register_value & 0xff00ffff) | (div_ratio << 16) | (1
									      <<
									      7);
		if (MAP_FAILED != pCSIReg) {
			munmap(pCSIReg, 256);
		...
		if ((gmap_fd > 0) && (close(gmap_fd) < 0)) {
			tst_resm(TFAIL, "Unable to close /dev/mam : %s",
				 strerror(errno));
			return TFAIL;
		}
		return TPASS;
