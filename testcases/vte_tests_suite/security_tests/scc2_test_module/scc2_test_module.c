...
scc2_test_get_configuration(unsigned long scc_data)
{
    ...
    scc_config_t *scc_config;

    scc_config = scc_get_configuration();
#ifdef SCC2_DEBUG
    printk("SCC2 TEST Configuration info:  \n  \ 
           scc_config_t.driver_major_version = %d \n  \  
           scc_config_t.driver_minor_version = %d \n  \
           scc_config_t.scm_version = %d \n   \
           scc_config_t.smn_version = %d \n   \
           scc_config_t.block_size_bytes = %d \n  \
           scc_config_t.partition_size_bytes = %d \n  \
           scc_config_t.partition_count = %d \n",scc_config->driver_major_version,scc_config->driver_minor_version,scc_config->scm_version,scc_config->smn_version,scc_config->block_size_bytes,scc_config->partition_size_bytes,scc_config->partition_count);

#endif

    /* now copy out (write) the data into user space */
    /** @todo make sure scc_get_configuration never returns null! */
    if (copy_to_user((void *)scc_data, scc_config,
                     sizeof(scc_config_t))) {
#ifdef SCC2_DEBUG
