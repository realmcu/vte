...
Blake/b19411                 25/07/2008                 add and update code for supporting sccv2.
======================
Total Tests: 1
...
int VT_scc2_test_setup(void)
{
        ...
        strcat(f_name, SCC2_TEST_DRIVER_NAME);

        if ((scc_fd = open(f_name, O_RDWR)) < 0)
        {
                VT_rv = TFAIL;
                tst_resm(TFAIL, "Failed open device %s", SCC2_TEST_DRIVER_NAME);
        }
        else
        {
                tst_resm(TPASS, "Device named %s is opened", SCC2_TEST_DRIVER_NAME);
        }
        return VT_rv;
}

/*====================*/
/*= VT_scc2_test_cleanup =*/
/**
...
int VT_scc2_test_cleanup(void)
{
        ...
        {
                close(scc_fd);
                tst_resm(VT_rv, "Device named %s is closed", SCC2_TEST_DRIVER_NAME);
        }
        else
        {
                VT_rv = TFAIL;
                tst_resm(VT_rv, "Failed close device %s", SCC2_TEST_DRIVER_NAME);
        }
        return VT_rv;
}
...
int check_safe_registers_test(void)
{
        ...
        uint32_t        value;
#if 0
        if (read_scc_register(SCM_STATUS_REG, &value) == TPASS)
        {
                tst_resm(TPASS, "(%08X) SCM Status     (0x%08x): ", SCM_STATUS_REG , value);
                print_scm_status_register(value);
        }
        else
        {
                VT_rv = TFAIL;
                tst_resm(TFAIL, "Could not get (0x%08X) SCM Status Register",SCM_STATUS_REG );
        }

        if (read_scc_register(SCM_ERR_STATUS_REG, &value) == TPASS)
        {
                tst_resm(TPASS, "(%08X) SCM Error Register     (0x%08x):",SCM_ERR_STATUS_REG, value);
                print_scc_error_status_register(value);
        }
        else
        {
                VT_rv = TFAIL;
                tst_resm(TFAIL, "Could not get (0x%08X) SCM Error Register",SCM_ERR_STATUS_REG);
        }

        if (read_scc_register(SCM_INT_CTL_REG, &value) == TPASS)
        {
...
/*   SCM_CONFIGURATION register is not exist for sccv2 */
...
/*
 @function    do_enc_dec
 @brief    Do the encrypt-decrypt part of the partition/cipher tests
 ...
 @author    Blake
*/
int do_enc_dec(int scc_fd, uint32_t part_virtual_addr)
{
    int status;
...
int run_cipher_tests(void)
{
    ...
    /*print info owned partition*/
    printf("owned partition info: \n");
...
int set_software_alarm_test(void)
{
        ...
        if (ioctl(scc_fd, SCC2_TEST_SET_ALARM, NULL) == SCC_RET_OK)
        {
...
int run_timer_tests(uint32_t timer_iv)
{
        ...
        if (write_scc_register(SMN_TIMER_IV_REG, timer_iv) == TPASS)
        {
        ...
        if (write_scc_register(SMN_TIMER_CTL_REG, SMN_TIMER_LOAD_TIMER) == TPASS)
        {
        ...
        if (read_scc_register(SMN_TIMER_REG, &value) == TPASS)
        {
        ...
        if (write_scc_register(SMN_TIMER_CTL_REG, SMN_TIMER_START_TIMER) == TPASS)
        {
        ...
        /* now stop the timer */
        if (read_scc_register(SMN_TIMER_CTL_REG, &value) == TPASS)
        {
        ...
        if (write_scc_register(SMN_TIMER_CTL_REG, value & ~SMN_TIMER_STOP_MASK) == TPASS)
        {
...
int run_aic_tests(void)
{
        ...
        if (write_scc_register(SMN_SEQ_START_REG, 4) == TFAIL)
        {
                VT_rv = TFAIL;
        ...
        if (write_scc_register(SMN_SEQ_END_REG, 5) == TFAIL)
        {
                VT_rv = TFAIL;
        ...
        if (write_scc_register(SMN_SEQ_CHECK_REG, 5) == TFAIL)
        {
                VT_rv = TFAIL;
        ...
        if (read_scc_register(SMN_STATUS_REG, &value) == TFAIL)
        {
                VT_rv = TFAIL;
                tst_resm(TFAIL, "Reading value from SMN_STATUS (0x%08X) failed",  SMN_STATUS_REG);
        }
        else
        {
                tst_resm(TPASS, "Reading value from SMN_STATUS (0x%08X) passed",  SMN_STATUS_REG);
        }
...
int write_scc_register(uint32_t reg, uint32_t value)
{
        ...
        status = ioctl(scc_fd, SCC2_TEST_WRITE_REG, &register_access);
...
int read_scc_register(uint32_t reg, uint32_t * value)
{
        ...
        register_access.reg_offset = reg;
        status = ioctl(scc_fd, SCC2_TEST_READ_REG, &register_access);
...
scc_configuration_access *get_scc_configuration(int scc_fd)
{
        ...
        if (NULL == config)
        {
                ...
                if (NULL != config)
                {
                        if (ioctl(scc_fd, SCC2_TEST_GET_CONFIGURATION, config) != SCC_RET_OK)
                        {
...
int scc_test_monitor_security_failure(void)
{
        int     VT_rv = TPASS;
#if 0
        if (ioctl(scc_fd, SCC2_TEST_SET_MON_SEC_FAIL, NULL) == SCC_RET_OK)
        {
                tst_resm(TPASS,
