...
static int __init rng_test_init(void)
{
        ...
        rng_class = class_create(THIS_MODULE, "rng_test_module");
        ...
        printk("rng : creating devfs entry for rng_test \n");

/*        if ((//---devfs_mk_cdev(MKDEV(major_dev_num, 0), S_IFCHR | S_IRUGO | S_IWUGO, RNG_DEVICE_NAME)) < 0)
        {
                printk(KERN_WARNING "rng Module Test: unable to create the dev\n");
                return -1;
        }
*/
...
err_out:
        ...
        unregister_chrdev(major_dev_num, "rng_test_module");
        return -1;
