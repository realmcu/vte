...
void setup(void)
{
        int     VT_rv = VT_scc_test_setup();

        if (VT_rv == TFAIL)
        {
                tst_brkm(TBROK, cleanup, "VT_scc_setup() failed : error code = %d", VT_rv);

        }
        else
        {
                tst_resm(TPASS, "VT_scc_setup() passed: code = %d", VT_rv);
        }
}
...
void help(void)
{
        printf( "Usage: scc_test [-T][-R offset][-W offset:value][-L tests][-S+/-options]\n"
                ...
                "     z (zeroize Red and Black memories of the SCC)\n"
                   "      d (check the register safe after do some operation)\n" 
        ...
        while ((argument_switch = getopt(arg_count, arg_list, "L:R:MS:TW:H")) != EOF)
        {
                ...
                case 'T':      /* change Timer Initial Value */
                        timer_value = 44;       /* code this up! */
                            print_smn_timer_control_register(timer_value);
                break;
