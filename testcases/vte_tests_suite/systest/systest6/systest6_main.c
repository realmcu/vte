...
void Help( void )
{
        ...
        printf("-T <Num> Thread to execute. Num must be in range [0..1]:\n");
        printf("         0 - Video playback;\n");
        printf("         1 - Transfer to/from flash card (over USB HS interface);\n");
        printf("-D <devname> V4L output device name. Default value is /dev/video0 \n");
        printf("-S <filename> Video stream to play. \n");
        printf("-M <dirname> FFS mounting point. Default value is ./output \n");
        printf("-F <devname> FFS device name to be mounted: SD/MMC card/USB HS. Default value is /dev/mmcblk0p1 (MMC card)\n");
}
...
int main( int argc, char ** argv )
{
        ...
        // Video file to play.
        int videoFileFlag = 0; char * videoFileOpt = 0;
        ...
        option_t options[] =
        {
                ...
                { "S:",  &videoFileFlag,            &videoFileOpt }, // video file name to be played
        ...
        {
                // Set to default.
                strncpy(gTestappConfig.mV4LDevName, "/dev/video16", MAX_STR_LEN);
        }
        ...
        if (videoFileFlag)
        {
                if (!videoFileOpt)
                {
                        tst_resm(TCONF, "%s(): -S argument is required", __FUNCTION__);
                        Help();
                        return TCONF;
                }
                strncpy(gTestappConfig.mVideoFileName, videoFileOpt, MAX_STR_LEN);
        }
        else
        {
                tst_resm(TCONF, "%s(): -S option is required", __FUNCTION__);
                Help();
                return TCONF;
        }
