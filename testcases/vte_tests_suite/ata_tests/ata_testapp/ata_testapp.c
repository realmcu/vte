                       ...
                       int flush_buffer)
{
        ...
        case 0:
                ...
                if (ioctl(fd, HDIO_SET_32BIT, io32bit) != TPASS)
                {
                        tst_resm(TFAIL, "HDIO_SET_32BIT failed. Error string: %s", strerror(errno));
                        VT_rv = TFAIL;
                }
                if (ioctl(fd, HDIO_GET_32BIT, &parm) == TPASS)
                {
                ...
                {
                        tst_resm(TFAIL, "HDIO_GET_32BIT failed. Error string: %s", strerror(errno));
                        VT_rv = TFAIL;
                }
                break;
        case 1:
                tst_resm(TINFO, "setting using_dma to %d", dma_flag);
                on_off(dma_flag);
                if (ioctl(fd, HDIO_SET_DMA, dma_flag) != TPASS)
                {
                        tst_resm(TFAIL, "HDIO_SET_DMA failed. Error string: %s", strerror(errno));
                        VT_rv = TFAIL;
                }
                if (ioctl(fd, HDIO_GET_DMA, &parm) == TPASS)
                {
                        if (parm != dma_flag)
                        {
                                tst_resm(TFAIL, "DMA flag wasn't setted");
                                VT_rv = TFAIL;
                        }
                        tst_resm(TINFO, "HDIO_GET_DMA returned: %ld", parm);
                }
                else
                {
                        tst_resm(TFAIL, "HDIO_GET_DMA failed. Error string: %s", strerror(errno));
                        VT_rv = TFAIL;
                }
                break;
        case 2:
                if (do_blkgetsize(fd, &blksize) == TPASS)
                {
                        if (ioctl(fd, HDIO_GETGEO, &g) != TPASS)
                        {
                                tst_resm(TFAIL, "HDIO_GETGEO failed. Error string: %s",
                                         strerror(errno));
                                VT_rv = TFAIL;
                        }
                        else
        ...
        case 5:
                ...
                break;
        case 6:
                if (ioctl(fd, HDIO_DRIVE_RESET, NULL) != TPASS)
                {
                        tst_resm(TFAIL, "HDIO_DRIVE_RESET failed. Error string: %s",
                                 strerror(errno));
                        VT_rv = TFAIL;
                }
                break;
