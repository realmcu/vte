...
void help(void)
{
        ...
        printf("\t  '-T 1 -F flag'           get/set using_dma flag(flag: 0,1)\n");
        printf("\t  '-T 2'                   display drive geometry\n");
        ...
        printf("\t  '-T 6'                   perform resetting a device(DANGEROUS)\n");
...
int main(int argc, char **argv)
{
        ...
        if (fflag != 0)
        {
                ...
                if(dma_flag>>1)
                {
                        tst_resm(TFAIL, "Arg -F: possible values are 0 and 1");
                        help();
                        return rv;
                }                 
        }
        ...
        {    
                if(testcase == 1) /* it is mandatory to set -F arg for testcase #1 */
                {
                        tst_resm(TFAIL, "Arg -F (dma flag) is not set");
                        help();
                        return rv;
                }
        }
        ...
        {    
                if(testcase == 7) /* it is mandatory to set -X arg for testcase #7 */
                {
