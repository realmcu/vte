...
@brief VTE C main source MMC/SD testcase extracting event during read/write process
...
/*======================== LOCAL VARIABLES ==================================*/
int test_case_name, d_num = 0, vb_mode = 0, c_num = 0, t_num = 0;
BOOLEAN read_flag = FALSE;
char *device_name, *c_copt, *t_copt;
unsigned long bcount;

option_t options[] = {
    { "D:", &d_num, &device_name },     /* MMC device name */
    { "T:", &t_num, &t_copt },	 	/* Test type */
    { "C:", &c_num, &c_copt },          /* Byte count */
...
/* Global Variables */
char *TCID     = "erw_mmc_test"; /* test program name                  */
...
void setup()
{
	...
	VT_rv = VT_erw_mmc_setup();
	if (VT_rv != TPASS)
	{
	    tst_brkm(TBROK , cleanup, "VT_erw_mmc_setup() Failed : error code = %d", VT_rv);
	}
    /* VTE */
    return;
}
...
void help()
{
    printf("Usage : %s -D <MMC block device> -C <block count> -V\n", TCID);
    return;
}
...
int main(int argc, char **argv)
{
   ...
   if (!d_num)
    ...
    if (t_num)
    {
        if (strcmp(t_copt, "READ") == 0)
            read_flag = TRUE;
        else
            if (strcmp(t_copt, "WRITE") == 0)
                read_flag = FALSE;
            else
    ...
    VT_rv = VT_erw_mmc_test(); /*with the parameters needed come from parse_opt())*/
...
void cleanup()
{
	...
	VT_rv = VT_erw_mmc_cleanup();
	if (VT_rv != TPASS)
	{
	    tst_resm(TWARN, "VT_erw_mmc_cleanup() Failed : error code = %d", VT_rv);
	}
    tst_exit();
}
