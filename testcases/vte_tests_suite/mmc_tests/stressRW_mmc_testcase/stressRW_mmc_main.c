...
void setup(void)
{
        ...
        VT_rv = VT_stressRW_mmc_setup();
        if (VT_rv != TPASS)
        {
                tst_brkm(TBROK, cleanup, "VT_stressRW_mmc_setup() Failed : error code = %d", VT_rv);
        }
}
