...
int VT_stressRW_mmc_setup(void)
{
        ...
        if (fd_device == -1)
        {
                tst_resm(TFAIL, "VT_stressRW_mmc_setup() Failed open device: %s", device_name);
                return TFAIL;
        }
        ...
        if (patten_buf == 0)
        {
                tst_resm(TFAIL, "VT_stressRW_mmc_test() Failed allocate memory");
                return TFAIL;
        }
...
int VT_stressRW_mmc_cleanup(void)
{
        ...
        if (fd_device > 0)
                ...
                {
                        tst_resm(TFAIL, "VT_stressRW_mmc_cleanup() Failed close device");
                        return TFAIL;
                }
