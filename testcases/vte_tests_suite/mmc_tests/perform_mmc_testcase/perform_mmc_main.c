...
void setup(void)
{
        ...
        VT_rv = VT_perform_mmc_setup();
        if (VT_rv != TPASS)
        {
                tst_brkm(TBROK, cleanup, "VT_perform_mmc_setup() Failed : error code = %d", VT_rv);
        }
}
...
void cleanup(void)
{
        ...
        VT_rv = VT_perform_mmc_cleanup();
        if (VT_rv != TPASS)
        {
                tst_resm(TWARN, "VT_perform_mmc_cleanup() Failed : error code = %d", VT_rv);
        }
        tst_exit();
}
