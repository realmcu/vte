...
static int gPixelFormat = V4L2_PIX_FMT_UYVY;
static char gPixFmtName[40] = "YUYV";
...
static TBuffer buffers[TEST_BUFFER_NUM];
...
int init_capture(void)
{
	...
	while (ioctl(gFdV4L, VIDIOC_ENUM_FMT, &fmtdesc) >= 0) {
		...
		while (ioctl( gFdV4L, VIDIOC_ENUM_FRAMESIZES, &fmtsize ) >= 0 ){
			...
			frmival.height = fmtsize.discrete.height;
			/* Enumerate frame intervals */
			while (ioctl(gFdV4L, VIDIOC_ENUM_FRAMEINTERVALS, &frmival) >= 0) {
				tst_resm(TINFO,"%.3f fps\n", 1.0 * frmival.discrete.denominator
	...
	if (fmtdesc.index == 0)
	{
		tst_resm(TWARN, "%s do VIDIOC_ENUM_FMT failed", gV4LTestConfig.mV4LDevice);
		return TFAIL;

	}
	if (fmtsize.index == 0)
	{
		tst_resm(TWARN, "%s do VIDIOC_ENUM_FRAMESIZES failed", gV4LTestConfig.mV4LDevice);
		return TFAIL;
    }
	if (frmival.index == 0)
    {
	...
	{
		tst_resm(TWARN, "%s Set PARM failed", gV4LTestConfig.mV4LDevice);
		return TFAIL;
	}
	/* Get original format */
	CLEAR(gFormat);
	...
	if(ioctl(gFdV4L, VIDIOC_G_FMT, &gFormat) < 0)
	{
		tst_resm(TWARN, "%s get formatting failed", gV4LTestConfig.mV4LDevice);
		return TFAIL;
	}
	...
	tst_resm(TINFO, "Pixel format %s is supported by device %s", gPixFmtName, gV4LTestConfig.mV4LDevice);
...
int init_uptr(void)
{
   ...
   if (memalloc(gFormat.fmt.pix.sizeimage, TEST_BUFFER_NUM) < 0) {
		tst_resm(TWARN, "phy malloc fails");
		return TFAIL;
	} 


	reqBuffers.count = TEST_BUFFER_NUM;
	reqBuffers.type = V4L2_BUF_TYPE_VIDEO_CAPTURE; 
	reqBuffers.memory = V4L2_MEMORY_USERPTR; 
	...
	{    
        ...
        buffer.length = buffers[gBuffNumber].length;
		buffer.m.userptr = (unsigned long) buffers[gBuffNumber].offset;
	...
	{    
		case V4L2_BUF_TYPE_VIDEO_CAPTURE:   
			...
			{           
				...
				buffer.memory =  V4L2_MEMORY_USERPTR;      
				buffer.index = i;
				buffer.m.offset = buffers[i].offset;
...
int process_image(const unsigned char *aStart, int aLength)
{
	...
	{     
		...
		case PRP_ENC_ON_D:  
			...
			if (gPixelFormat != V4L2_PIX_FMT_YUYV && gPixelFormat != V4L2_PIX_FMT_YUV420 && V4L2_PIX_FMT_UYVY != gPixelFormat)
			{
				tst_resm (TWARN, "Unsupported pixel format: %s : %x", gPixFmtName, gPixelFormat);
				tst_resm (TINFO, "Framebuffer pixel format: %s", gFBPixFmtName);
				return TFAIL; 
			}
			...
			}else if (gPixelFormat == V4L2_PIX_FMT_UYVY){
				;
			}else{
				/* Buggie temp set display to FB directly for this format */
				display_to_fb((void*)aStart, aLength);
				//memcpy(gpFB,aStart,aLength);
				break;
			}
...
{  
	...
	switch(gUsecase)
	{
		case V4L2_BUF_TYPE_VIDEO_CAPTURE:
			...
			tst_resm(TINFO,"read_frame() for display");
			...
			{        
				...
				aStart = buffers[buffer.index].start;

			if(process_image (aStart,buffers[buffer.index].length) == TFAIL) 
				return TFAIL;
			/*
			   if (ioctl(gFdFB, FBIOPAN_DISPLAY, &gScreenInfo) < 0) {
...
int process_capture (void)
{
	...
	tst_resm(TINFO,"finish capturing..."); 
	return TPASS; 
}
/*===== process_video =====*/
/**
