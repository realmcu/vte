...
int VT_usb_ptp_test(int switch_fct){
 ...
 switch(switch_fct)
 {
  ...
  case 6:
   ...
   rv=SendFile(filenameInput, idInput);
   break;
  case 7:
   ...
   if (selectionInput == '1') {
    printf("Try to set protection for file 0x%x\n",
           idInput);
    rv=SetFileProtection(idInput, 1);
   } else {
    printf
        ("Try to clear protection for file 0x%x\n",
         idInput);
    rv=SetFileProtection(idInput, 0);
   }
   break;
  case 8:
