/*====================*/
/**
    @file   dio_sparse_main.c

    @brief  Main file of dio_sparse test application.
*/
/*======================
...
Test Name:      dio_sparse_test

Test Assertion
& Strategy:     dio_sparse - issue O_DIRECT writes to holes is a file while concurrently
...
/* Global Variables */
char *TCID     = "dio_sparse_test";   /* test program identifier.          */
...
void cleanup()
{
    ...
    VT_rv = VT_dio_sparse_cleanup();
    if (VT_rv != TPASS)
    {
 tst_resm(TWARN, "VT_dio_sparse_cleanup() Failed : error code = %d", VT_rv);
    }
    /* VTE */
...
void setup()
{
    ...
    VT_rv = VT_dio_sparse_setup();
    if (VT_rv != TPASS)
    {
 tst_brkm(TBROK , cleanup, "VT_dio_sparse_setup() Failed : error code = %d", VT_rv);
    }
    /* VTE */
...
int main(int argc, char **argv)
{
    ...
    VT_rv = VT_dio_sparse_test(argc,argv); /*with the parameters needed come from parse_opt())*/
