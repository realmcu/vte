...
/**
    ...
    @brief  Main file of dio_bio Test Application
*/
/*======================
...
Test Name: dio_bio_test

Test Assertion
& Strategy: Program tests the combinations of direct and buffered read, write.
...
/* Global Variables */
char *TCID     = "dio_bio_test";   /* test program identifier.          */
...
void cleanup()
{
    ...
    VT_rv = VT_dio_bio_cleanup();
    if (VT_rv != TPASS)
    {
 tst_resm(TWARN, "VT_dio_bio_cleanup() Failed : error code = %d", VT_rv);
    }
...
void setup()
{
    ...
    VT_rv = VT_dio_bio_setup();
    if (VT_rv != TPASS)
    {
 tst_brkm(TBROK , cleanup, "VT_GPIO_test2_setup() Failed : error code = %d", VT_rv);
    }
...
int main(int argc, char **argv)
{
    ...
    VT_rv = VT_dio_bio_test(argc, argv);
