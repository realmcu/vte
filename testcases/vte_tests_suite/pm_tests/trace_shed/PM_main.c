...
int threads_flag = 0;
int policy_flag = 0;
char *policy_option=0;
char *threads_option=0;
char *cpu_option=0;
...
option_t options[] =
  {
    { "v", &verbose_flag, NULL },
    { "C:", &cpu_flag, &cpu_option },
    { "S:", &policy_flag, &policy_option },
    { "T:", &threads_flag, &threads_option },
...
/* Global Variables */
char *TCID     = "trace_shed_testapp"; /* test program identifier.          */
...
void help()
{
  printf("Options:\n\n");
  printf(
	"\t -C Number of CUPS in the machine.              Default: 1\n"
	"\t -S Scheduling policy, choice: fifo, rr, other. Default: fifo\n"
...
int main(int argc, char **argv)
{
  ...
  if (msg != (char *)NULL)
    ...
    VT_rv = VT_PM_test_X(cpu_option,threads_option,policy_option); /*with the parameters needed come from parse_opt())*/
