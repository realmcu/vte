...
#define ADV_PWR                        91       /* current advanced power management level in low
                                                * byte, 0x40 in high byte. */
...
/* word 2: specific configureation (a) require SET FEATURES to spin-up (b) require spin-up to fully
* reply to IDENTIFY DEVICE */
...
#define STD_STBY                    0x2000      /* 1=standard values supported (ATA); 0=vendor
                                                * specific values */
...
#define MODE_MAX                7       /* bit definitions force udma <=7 (when * udma >=8 comes out
                                        * it'll have to be * defined in a new dma_mode word!) */
...
/* use cmd_feat_str[] to display what commands and features have been enabled with words 85-87 */
...
void identify(__u16 * id_supplied, const char *devname)
{
        ...
        /* Info from the specific configuration word says whether or not the ID command completed
        * correctly.  It is only defined, however in ATA/ATAPI-5 & 6; it is reserved (value
        * theoretically 0) in prior standards.  Since the values allowed for this word are
        * extremely specific, it should be safe to check it now, even though we don't know yet what
        * standard this device is using. */
        ...
        /* major & minor standards version number (Note: these words were not defined until ATA-3 &
        * the CDROM std uses different words.) */
        printf("Standards:");
        if (eqpt != CDROM)
        {
                ...
                /* looks like when they up-issue the std, they obsolete one; thus, only the newest 4
                * issues need be supported. (That's what "kk" and "min_std" are all about) */
                ...
                * happens. */
                ...
                if (((std == 5) || (!std && (like_std < 6))) &&
                    ((((val[CMDS_SUPP_1] & VALID) == VALID_VAL) &&
                     ...
                     (((val[CMDS_SUPP_2] & VALID) == VALID_VAL) && (val[CMDS_SUPP_2] & CMDS_W84))))
                {
                        like_std = 6;
                }
        ...
        else
        {
                ...
                if (val[CAPAB_0] & LBA_SUP)
                {
                            ...
                            (val[CMDS_SUPP_1] & SUPPORT_48_BIT))
                        {
                                bbbig = (__u64) val[LBA_64_MSB] << 48 |
                                    ...
                                    (__u64) val[LBA_MID] << 16 | val[LBA_LSB];
                ...
                printf("\tdevice size with M = 1000*1000: %11llu MBytes ",
                       (unsigned long long) bbbig);
                if (bbbig > 1000)
        ...
        {
                printf("IORDY");
                if (!(val[CAPAB_0] & IORDY_SUP))
                        printf("(may be)");
                if (val[CAPAB_0] & IORDY_OFF)
                        printf("(can");
                else
                        printf("(cannot");
                printf(" be disabled)");
        ...
        if ((like_std == 1) && val[BUF_TYPE])
        {
                ...
                if (kk < 2)
                        printf("single port, single-sector");
                else
                        printf("dual port, multi-sector");
                if (kk > 2)
                        printf(" with read caching ability");
                printf("\n");
        }
        jj = 0;
        ...
        if (dev == ATA_DEV)
        {
                ...
                else
                {
                        ...
                        if (val[CAPAB_0] & STD_STBY)
                                printf("Standard");
                        else
                                printf("Vendor");
                        if ((like_std > 3) && ((val[CAPAB_1] & VALID) == VALID_VAL))
                        {
                                if (val[CAPAB_1] & MIN_STANDBY_TIMER)
                                        printf(", with ");
                                else
                                        printf(", no ");
        ...
        {       /* ATAPI */
                ...
                if (val[PKT_REL] || val[SVC_NBSY])
                {
                        ...
                        if (val[SVC_NBSY])
                                printf(" %uus to clear BSY after SERVICE cmd.", val[SVC_NBSY]);
                        printf("\n");
                }
        }
        ...
        else
        {
                ...
                if ((val[WHATS_VALID] & OK_W64_70) && (val[DMA_TIME_MIN] || val[DMA_TIME_NORM]))
                {
                        printf("\t     Cycle time:");
                        if (val[DMA_TIME_MIN])
        ...
        /* Programmed IO stuff */
        printf("\tPIO: ");
        /* If a drive supports mode n (e.g. 3), it also supports all modes less than n (e.g. 3, 2, 1
        * and 0).  Print all the modes. */
            ...
            (val[SECU_STATUS] || val[ERASE_TIME] || val[ENH_ERASE_TIME]))
        {
                ...
                if (jj || kk)
                {
                        printf("\t");
                        if (jj)
                                printf("%umin for SECURITY ERASE UNIT. ",
                                       jj == ERASE_BITS ? 508 : jj << 1);
                        if (kk)
                                printf("%umin for ENHANCED SECURITY ERASE UNIT.",
                                       kk == ERASE_BITS ? 508 : kk << 1);
                        printf("\n");
                }
        }
        ...
        if ((val[HWRST_RSLT] & VALID) == VALID_VAL)
        {
                ...
                if (val[HWRST_RSLT] & CBLID)
                        printf("\tCBLID- above Vih\n");
                else
                        printf("\tCBLID- below Vih\n");
        ...
        if ((like_std > 4) && (eqpt != CDROM))
        {
                if (val[CFA_PWR_MODE] & VALID_W160)
                {
                        ...
                        if (val[CFA_PWR_MODE] & PWR_MODE_OFF)
                                printf("dis");
                        else
                                printf("en");
                        printf("abled");
