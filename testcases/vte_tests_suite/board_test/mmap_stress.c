/*
 * mmap_stress.c - Tests mmapping a big file and writing it once
 *
...
int main(int argc, char *argv[])
{
	...
	for (lc = 0; TEST_LOOPING(lc); lc++) {
		...
		if (STD_FUNCTIONAL_TEST) {
			...
			/*
			 * now all the memory is mmaped, and physically allocated,
			 * so we can generate random access sequences as below:
			 * 1. read byte/word/dw from random.
			 * 2. write byte/word/dw to random.
			 * 3. read from start and write to the end for 100pages(if any).
			 * 4. memory copy form  
			 */
