#Copyright 2008-2010 Freescale Semiconductor, Inc. All Rights Reserved.
#
#The code contained herein is licensed under the GNU General Public
#License. You may obtain a copy of the GNU General Public License
#Version 2 or later at the following locations:
#
#http://www.opensource.org/licenses/gpl-license.html
#http://www.gnu.org/copyleft/gpl.html
#!/bin/sh
##############################################################################
#
# Revision History:
#                          Modification     Tracking
# Author                       Date          Number    Description of Changes
#-----------------------   ------------    ----------  ---------------------
# Spring Zhang               03/07/2008       n/a        Initial ver. 
# Spring                     28/11/2008       n/a      Modify COPYRIGHT header
# Spring                     01/04/2010       n/a      Correct return value caused by two version
#############################################################################
# Portability:  ARM sh bash 
#
# File Name:    fs_perms_list.sh
# Total Tests:        21
# Test Strategy: file access permission test
# 
# Input:	none
#
# Return:     
#
# Use command "./fs_perms_list.sh to test file system access permission


# Function:     main
#
# Description:  - Execute all tests, exit with test status.
#
# Exit:         - zero on success
#               - the last error test number on failure.
#
#####################################
# Test if test.txt testx.file testx exsit!
#####################################
if [ ! -e "test.txt" ] || [ ! -e "testx.file" ] || [ ! -e "testx" ]
then
    tst_resm TBROK "Test #1: test.txt, testx.file or testx don't exist!"
    exit 255
fi


# return value
RC=0
# the last error test number
last_err=0

fs_perms 001 99 99 12 100 x 0 \
|| last_err=`expr $last_err + 1`
sleep 1

fs_perms 010 99 99 200 99 x 0 \
|| last_err=`expr $last_err + 1`
sleep 1

fs_perms 100 99 99 99 500 x 0 \
|| last_err=`expr $last_err + 1`
sleep 1

fs_perms 002 99 99 12 100 w 0 \
|| last_err=`expr $last_err + 1`
sleep 1

fs_perms 001 99 99 12 100 x 0 \
|| last_err=`expr $last_err + 1`
sleep 1

fs_perms 020 99 99 200 99 w 0 \
|| last_err=`expr $last_err + 1`
sleep 1

fs_perms 200 99 99 99 500 w 0 \
|| last_err=`expr $last_err + 1`
sleep 1

fs_perms 004 99 99 12 100 r 0 \
|| last_err=`expr $last_err + 1`
sleep 1

fs_perms 040 99 99 200 99 r 0 \
|| last_err=`expr $last_err + 1`
sleep 1

fs_perms 004 99 99 12 100 r 0 \
|| last_err=`expr $last_err + 1`
sleep 1

fs_perms 004 99 99 12 100 r 0 \
|| last_err=`expr $last_err + 1`
sleep 1

fs_perms 000 99 99 99 99 w 1 \
|| last_err=`expr $last_err + 1`
sleep 1

fs_perms 000 99 99 99 99 x 1 \
|| last_err=`expr $last_err + 1`
sleep 1

fs_perms 010 99 99 99 500 x 1 \
|| last_err=`expr $last_err + 1`
sleep 1

fs_perms 100 99 99 200 99 x 1 \
|| last_err=`expr $last_err + 1`
sleep 1

fs_perms 020 99 99 99 500 w 1 \
|| last_err=`expr $last_err + 1`
sleep 1

fs_perms 200 99 99 200 99 w 1 \
|| last_err=`expr $last_err + 1`
sleep 1

fs_perms 040 99 99 99 500 r 1 \
|| last_err=`expr $last_err + 1`
sleep 1

fs_perms 400 99 99 200 99 r 1 \
|| last_err=`expr $last_err + 1`
sleep 1
# Two additional
fs_perms 755 50084 1026 0 0 r 0 \
|| last_err=`expr $last_err + 1`
sleep 1

fs_perms 755 50084 1026 0 0 w 0 \
|| last_err=`expr $last_err + 1`
sleep 1


# exit value
if [ $last_err -ne 0 ]
then
    exit $last_err
fi

