...
/* RFC 3542, Section 4 */
 { "IPV6_RECVPKTINFO", IPV6_RECVPKTINFO, 1, IPV6_PKTINFO, 1,
  .so_clrval.sou_bool0, .so_setval.sou_bool1, sizeof(int) },
...
struct cme {
 ...
 { sizeof(uint32_t), SOL_IPV6, IPV6_TCLASS, .cmu.cmu_tclass0x12 },
 { sizeof(uint32_t), SOL_IPV6, IPV6_HOPLIMIT, .cmu.cmu_hops0x21 },
};
...
/* check pid & seq here */
  ...
  for (len0; len < msg.msg_controllen; len + pcmsg->cmsg_len) {
   if (!psoe->so_dorecv)
    break;
...
int getsock(char *tname, struct sockaddr_in6 *psin6_arg, int scope)
{
 ...
 for (pifapifa_head; pifa; pifapifa->ifa_next) {
  int this_scope;
  ...
  if (psin6_arg) {
   *psin6_arg  *psin6;
   psin6_arg->sin6_scope_idif_nametoindex(pifa->ifa_name);
  }
  return s;
 }
 {
