...
static long calc_time(int size)
{
	long ms, speed;
...
/*kmalloc test for 128k-16*/
  ...
  speed = calc_time(kmalloc_size * 10);
...
/*__get_free_pages */ 
  ...
  while(count--){
	...
	do_gettimeofday(&finish);
  speed = calc_time((2^freepage_order)*PAGE_SIZE * 10);
  printk(KERN_INFO "free apge memroy speed is %ld KB/s\n", speed);  
  free_pages(kernel_memaddr,10);
