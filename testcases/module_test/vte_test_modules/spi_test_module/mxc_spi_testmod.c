...
static int __devinit spi_lb_probe(struct spi_device *spi)
{
	...
	if (!strcmp(spi->dev.bus_id, "spi1.3")) {
		master = 1;
	} else if (!strcmp(spi->dev.bus_id, "spi2.3")) {
		master = 2;
	} else if (!strcmp(spi->dev.bus_id, "spi3.3")) {
		master = 3;
	}
	...
	if (IS_ERR(device_create(spi_class, NULL, NULL,
					   MKDEV(major_spi, master),
					   dev_name))) {
...
static int __devexit spi_lb_remove(struct spi_device *spi)
{
	...
	kfree(lb_drv_data[master]);
	device_destroy(spi_class, MKDEV(major_spi, master));
