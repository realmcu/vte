 ...
 * Watchdog driver test for Freescale mxs socs
 *
...
static struct resource mxs_wdt_resources[] = {
	{
		.start = RTC_PHYS_ADDR,
...
{
	  if(mxs_wdt_data.enable == 1)
				wdt_ping();
		mod_timer(mxs_wdt_data.timer, jiffies + HZ * 1);
}
...
static int __devinit mxs_wdt_probe(struct platform_device *pdev)
{
	...
	if (res == NULL)
		return -ENODEV;
#else
  res = &(mxs_wdt_resources[0]);
  res->start = RTC_PHYS_ADDR;
	res->end = RTC_PHYS_ADDR + 0x2000 - 1;
#endif
...
#else
	printk(KERN_INFO "mxs watchdog test: initialized, heartbeat %d sec\n",
		heartbeat);
  ...
  timer = kmalloc(sizeof(*timer), GFP_KERNEL);
	...
	add_timer(timer);
  mxs_wdt_data.timer = timer;
	mxs_wdt_data.enable = 1;
	if (test_and_set_bit(WDT_IN_USE, &wdt_status))
		return -EBUSY;
	clear_bit(WDT_OK_TO_CLOSE, &wdt_status);
  wdt_ping();
...
#else
	int ret = 0;
  del_timer(mxs_wdt_data.timer);
	kfree(mxs_wdt_data.timer);
	set_bit(WDT_OK_TO_CLOSE, &wdt_status);
	if (WATCHDOG_NOWAYOUT == 0) {
		if (!test_bit(WDT_OK_TO_CLOSE, &wdt_status)) {
			wdt_ping();
